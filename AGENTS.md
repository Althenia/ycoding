...
- After changing the macOS computer-use app or its permission requests, rebuild it with `bun run build:computer-use`, remove any existing `YCoding Computer Use` Privacy & Security entries, enable the rebuilt app as each run adds it, and run `YCODING_TEST_DESKTOP_TARGET=<bundle-id>/<pid>/<window-id> bun run --cwd packages/core test:integration:computer` with fresh IDs for a window on another Space; its desktop cases fail without that target.
