...
This contract specifies temporary headless browsing on macOS arm64 with installed Chrome major 152 or newer for the existing TUI product. It does not replace the [paired Chrome-profile contract](./browser.md), provide a desktop browser product, or establish release readiness from proof-of-concept results.
...
- Supported hosts require macOS arm64 with installed Google Chrome major version 152 or newer. Older versions and other hosts remain unavailable. Every launch still configures and verifies the required private-pipe safety controls before exposing a target; version eligibility alone does not establish their availability.
...
The paired Chrome extension and isolated executor are distinct supported modes, not interchangeable targets. A Session must explicitly stop its active mode before starting the other. Conflicting startup or reconnect must fail without replacing the active target or revoking unrelated trust.

Isolated startup neither consumes nor revokes extension credentials. Isolated stop affects only its own process and temporary resources. Paired Chrome stop retains its established trust semantics. Forget remains an explicit extension-trust operation and is not an isolated-browser lifecycle operation.
...
The dialog displays current state and offers valid explicit controls without double submission. Dismissing the dialog does not imply that the backend stopped. A lost start response triggers status reconciliation, not a second start or paired Chrome fallback. Stale responses after Session/Location changes must not update the new view. Fetch failure must not retain a misleading ready state.
...
| Compatibility       | Extension pairing, reconnect, stop, forget, profile-tab grants, and generated clients retain their meanings.                                                                                  |
...
Real-browser integration has been verified on macOS arm64 with Chrome 152 and 153. The Bun candidate smoke also passed on Chrome 153; the current range change has not been verified with a freshly built Node SEA candidate. Missing/incompatible installations are represented by unavailable executor fixtures; other host environments and later Chrome majors have not been runtime-validated. Tests do not inspect or interact with personal Chrome profiles. Rebuild and rerun packaged candidates after runtime changes.

On macOS arm64 with Chrome 152 or newer installed, run the isolated-browser smoke harness against each freshly built candidate. From `packages/cli`:
...
Release notes must distinguish isolated actions from paired Chrome-profile actions; the isolated executor never attaches to a personal Chrome profile.
