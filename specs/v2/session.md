...
Manual compaction does not use the prompt pending store. `POST /api/session/:sessionID/compact` accepts an optional `cmp_` idempotency ID and returns a pending durable `SessionCompaction.Admission` for a background job. The handler does not wait for manifest generation or activation.
...
Selective-compaction policy is measured against the hard input cap `context - output - context_safety_margin_tokens`. Advisory thresholds default to 70% (`consider`) and 90% (`advised`), can be configured as ordered integer percentages, and can be disabled with `compaction.advisory: false`. Advisory messages are volatile and receive no cache breakpoint. At either advisory level, `conversation_compact` accepts no input and durably admits a background job for the latest complete-message boundary; it returns `{ jobID, status: "scheduled" }` without waiting. Direct manual admission uses the same job authority and a `manual` trigger.

A job captures its requested boundary, base context revision, target input budget, and configuration digest. The worker leases one job per Session, resolves the Session Location at execution, and can recover pending or expired leased work through the restart-recovery action. A waiter also re-wakes nonterminal work before each one-second settlement poll, allowing pending work or a running job whose foreign 30-second lease expired to be claimed without a separate recovery call. Core owns every selector and the complete manifest structure. The optional helper emits only strict `conversation_memory` TOON content in bounded batches; empty, malformed, oversized, timed-out, provider-failed, or model-history-growing output is replaced by a deterministic local canonical checkpoint from the same source messages. The local checkpoint uses the newest source text that fits after its fixed fields. Core measures the shared synthetic checkpoint plus retained later entries against the selected pre-compaction model history and records both counters in the manifest; when `max_manifest_bytes` is smaller than the minimal canonical TOON document, the minimal document wins, but activation still requires token reduction. Advisory and manual jobs retain at most `compaction.keep_recent_messages`; mandatory jobs protect no recent tail and may summarize through the full captured boundary. Validation rejects stale revisions or boundaries, changed protected state, protected-tail overlap, dependency breakage, and zero-reduction candidates. `context_limit_unresolved` means even trusted checkpointing cannot reduce selected model history. `invalid_manifest` identifies persisted or invariant corruption, not ordinary helper output.
...
Current lifecycle events are `session.compaction.admitted.2`, `.started.2`, `.ended.2`, and `.failed.2`, projected by `jobID` into pending, running, completed, or failed messages. A started job has exactly one terminal event. Completed data includes revision, boundary, and excluded-message, excluded-part, input-token, and retained-token metrics. Failure codes are `cancelled`, `superseded`, `invalid_manifest`, `protected_state_changed`, `context_limit_unresolved`, `migration_failed`, or `provider_failed`. TUI reconciliation preserves one row across event-before-fetch, reconnect, and terminal updates and never renders opaque provider state or response IDs.
...
Before an ordinary provider request, the runner enforces the absolute safe input cap `context - maximum output - compaction.context_safety_margin_tokens`. At or above the cap it joins only an exact compatible pending or running job (same Session, base revision, configuration digest, boundary, and equal-or-stricter target) without charging that work to the gate's admission budget; an incompatible running job serializes admission only. Every terminal job triggers a rebuilt estimate: `ended` uses `fullRebase: true`, while `failed` uses `fullRebase: false`. If the estimate remains at or above the cap, the gate may own at most two mandatory admissions, the initial job plus one replacement, before one final `fullRebase: false` reload and deterministic `context.limit` failure. Mandatory admissions target the configured consider headroom even when advisory compaction is disabled.

The admission-and-wait workflow has one 61-second foreground deadline. Timeout interrupts and unregisters only the local waiter and releases the admission gate; it never cancels or fails shared compaction work, which continues independently in the background. The runner then reloads with `fullRebase: false` and proceeds only when the rebuilt estimate is strictly below the cap. A non-positive cap or absent complete boundary also fails before `llm.stream`. Stable prefixes or protected live state can make reduction irreducibly impossible; the Step then fails promptly with `context.limit`, the coordinator settles, and a later prompt or resume may retry. No ordinary provider request may be sent at or above the cap. A later provider overflow may trigger this gate once, then ends as `context.limit` if the provider still rejects the rebase.
