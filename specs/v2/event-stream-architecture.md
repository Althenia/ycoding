...
Protocol continues to own the `YCodingEvent` SSE contract. Generated Promise and Effect clients remain unchanged.
...
2. `YCodingEvent` schema encoding.
3. `JSON.stringify`.
4. SSE framing.
5. UTF-8 encoding.
...
An experiment replacing direct schema encoding plus `JSON.stringify` with `Schema.fromJsonString(YCodingEvent)` was discarded: the one-client median regressed from approximately 9.5 ms to 38.8 ms with substantially higher variance.
