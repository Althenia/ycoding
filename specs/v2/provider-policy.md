...
Both `action` and `resource` use ycoding's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.ycoding` files is intentionally deferred until `.ycoding` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether ycoding permits an operation through its managed execution paths.
