...
- Canonical tagged tool file sources in `@ycoding-ai/ai`.
...
- Spill oversized model-facing tool text into globally unique files under YCoding's shared tool-output directory.
...
- Canonicalize ambient instruction traversal boundaries, honor `YCODING_DISABLE_PROJECT_CONFIG`, and make non-empty aggregate updates explicitly supersede previously loaded instructions.
