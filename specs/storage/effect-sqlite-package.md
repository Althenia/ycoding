...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not a YCoding storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/core` uses it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No YCoding paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- YCoding-specific path and channel selection stays in `packages/core`.

## YCoding adoption notes

These are not package requirements, but they matter when YCoding consumes this adapter.

The current database layer in `packages/core/src/database` has two non-obvious semantics that the YCoding wrapper must preserve when it consumes this adapter:
...
The YCoding wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving YCoding to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@ycoding-ai/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@ycoding-ai/effect-drizzle-sqlite` as a dependency of `packages/core`.
5. Port `packages/core/src/database` to be a thin compatibility wrapper over the adapter plus YCoding-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of YCoding's database layer.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then YCoding can build its database layer on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay YCoding-specific until event publishing moves? Default answer: yes.
...
- Do CLI and administrative raw SQL capabilities stay in `packages/core`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not YCoding domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Keep YCoding domain migration work in `packages/core` and validate this adapter independently first.

That gives us a focused place to validate the Effect SQLite approach before disturbing YCoding's database runtime.
