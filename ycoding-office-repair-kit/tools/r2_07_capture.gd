...
func _measure_and_capture() -> void:
	...
	# The cluster's REGION must be able to hold what the cluster declares it needs at
	# this scale. When the region is narrower than a single row (which is legitimate:
	# the cluster wraps there) it must instead have been given the extra height for
	# the rows that width forces. A region that is both too narrow AND too short is
	# clipping its own content, which is what this check exists to catch.
	var toggles_rect: Rect2 = overlays["toggles"]
	var one_row_need := ChromeToggles.cluster_width(scale)
	var one_row_high := ChromeToggles.cluster_height(scale)
	if toggles_rect.size.x + 0.5 < one_row_need and toggles_rect.size.y + 0.5 <= one_row_high:
		_problems.append(
			"%s: the cluster region is narrower than one row (%.0f < %.0f) but not tall "
			% [label, toggles_rect.size.x, one_row_need]
			+ "enough to wrap (%.0f <= %.0f)" % [toggles_rect.size.y, one_row_high]
		)

	# A real Control must be at least its own minimum, or it clips its content.
	#
	# This check is WEAK BY CONSTRUCTION and is kept only as a cheap tripwire: Godot
	# clamps a laid-out Control's size UP to `get_combined_minimum_size()`, so a
	# control that cannot fit its content is normally reported larger rather than
	# smaller. The check that actually catches clipping is the geometric one above -
	# whether the RESERVED REGION is smaller than what the control declares it needs -
	# and the wrap assertions in test_shell_layout.gd.
