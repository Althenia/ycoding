...
func _run(scene) -> void:
	...
	# An ADVISORY BUDGET, set through the surface's own store. It must render apart from the
	# provider windows and must say it is the user's own policy.
	var store := QuotaBudgetStore.new()
	store.file_path = "user://r7_05_capture_budgets.cfg"
	DirAccess.remove_absolute(ProjectSettings.globalize_path(store.file_path))
	var budget := QuotaBudget.from_fields({
		"scope": QuotaBudget.SCOPE_PROVIDER, "scope_id": "openrouter",
		"unit": "usd", "limit": 50.0, "period_seconds": 604800, "warn_at": 0.8,
	})
	scene.statistics_panel.budget_store = store
	scene.statistics_panel.observe_spend("provider", "openrouter", 44.31, true)
	scene.statistics_panel.set_budget(budget)
	await process_frame
	await process_frame
	print("R704 budget label=%s" % budget.label())
	print("R704 budget advisory=%s enforced=%s" % [
		str(budget.is_advisory()), str(budget.is_enforced())])
	var verdict := budget.evaluate(44.31, true)
	print("R704 budget verdict=%s warns=%s" % [verdict.text(), str(verdict.should_warn())])
	# The unknown-spend case: the comparison must be PARTIAL, never "under budget".
	var unknown := budget.evaluate(0.0, false)
	print("R704 budget unknown=%s" % unknown.text())
	_check(
		not verdict.text().to_lower().contains("provider limit"),
		"the budget's own text does not claim to be a provider limit"
	)
	_check(
		unknown.is_known() == false and not unknown.text().to_lower().contains("within the budget"),
		"an unknown spend makes the comparison partial, not passing (%s)" % unknown.text()
	)

	var shown := _panel_text(scene.statistics_panel)
	print("R704 panel_names_quota=%s" % str(shown.contains("Provider quota")))
	print("R704 panel_names_budget=%s" % str(shown.contains("advisory")))
	_check(
		shown.contains("advisory") or shown.contains("Your advisory"),
		"the budget's own advisory label is on the surface"
	)
