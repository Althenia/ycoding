...
import { defineScript, Llm } from "ycoding-drive"
  ...
  run: ({ artifacts, llm, server }) =>
    Effect.gen(function* () {
      ...
      const journey = Effect.gen(function* () {
        yield* Effect.uninterruptible(
          Effect.promise(() =>
            tmux([
              ...
              `YCODING_PASSWORD=${registration.password}`,
              `YCODING_CONFIG_DIR=${path.join(artifacts, "files/.ycoding")}`,
              `YCODING_TEST_HOME=${artifacts}`,
        ...
        const first = yield* Effect.promise(() => waitForPane(session, "YCoding"))
        ...
        if (!exited.includes("Continue") || !exited.includes("ycoding mini -s"))
...
async function configureServicePort(artifacts) {
  ...
  const file = path.join(artifacts, "files/.ycoding/service-local.json")
...
async function serviceRegistration(artifacts) {
  const directory = path.join(artifacts, "home/.local/state/ycoding")
