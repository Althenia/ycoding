...
describe("CloudflareRemoteTransport", () => {
  test("requires a secure remote URL and rejects sends before opening", async () => {
    expect(() => new CloudflareRemoteTransport({ url: "ws://example.com/ws/v2/agent" })).toThrow(
      "Remote transport requires WSS",
    )
    expect(() => new CloudflareRemoteTransport({ url: "ws://127.0.0.1:8787/ws/v2/agent" })).not.toThrow()
    const transport = new CloudflareRemoteTransport({
      url: "wss://ycoding-cloud.lostq901.workers.dev/ws/v2/agent",
  ...
  test("passes connection headers to the socket factory and reports every open and unexpected close", async () => {
    ...
    const closes: number[] = []
    const transport = new CloudflareRemoteTransport({
      url: "wss://ycoding-cloud.example/ws/v2/agent",
    ...
    expect(factoryCalls).toEqual([
      { url: "wss://ycoding-cloud.example/ws/v2/agent", headers: { authorization: "Bearer access-token" } },
    ])
  ...
  test("carries the bearer upgrade header over a real socket", async () => {
    ...
    const transport = new CloudflareRemoteTransport({
      url: `ws://127.0.0.1:${server.port}/ws/v2/agent`,
