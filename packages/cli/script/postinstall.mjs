...
if (!command) throw new Error("YCoding package does not declare a binary")
...
if (!base) throw new Error(`YCoding does not provide a binary for ${platform}-${arch}`)
...
function installPackage(name) {
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "ycoding-install-"))
  try {
...
function main() {
  ...
  throw new Error(`Failed to install YCoding. Try manually installing ${names.map((name) => JSON.stringify(name)).join(" or ")}.`)
}
