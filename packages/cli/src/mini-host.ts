...
import { createModelPreferenceRepository } from "@ycoding-ai/tui/model-preference"
...
export const INTERACTIVE_INPUT_ERROR = "ycoding mini requires a controlling terminal for input"
...
function createTrace(
  ...
  if (!process.env.YCODING_DIRECT_TRACE) return
...
export function createMiniHost(input: {
  ...
  return {
    ...
    editor: {
      async open(options) {
        const { openEditor } = await import("@ycoding-ai/tui/editor")
    ...
    startup: {
      showTiming: ["1", "true"].includes(process.env.YCODING_SHOW_TTFD?.toLowerCase() ?? ""),
      now: () => performance.now(),
    },
    diagnostics: {
