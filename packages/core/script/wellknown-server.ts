...
const config = {
  ...
  disabled_providers: [
    "opencode", // YCODING_EXTERNAL_OPENCODE: external provider ID
    "anthropic",
    "openai",
    "google",
    "xai",
...
const server = Bun.serve({
  ...
  async fetch(request) {
    ...
    if (url.pathname === "/.well-known/ycoding") {
      return Response.json({
        ...
        remote_config: {
          url: `${url.origin}/config/ycoding.json`,
          headers: { authorization: "Bearer {env:TOKEN}" },
        },
      })
    }
    if (url.pathname === "/config/ycoding.json") {
