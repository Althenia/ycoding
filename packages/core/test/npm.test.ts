...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@example/acme")).toBe("@example/acme")
    expect(Npm.sanitize("@example/acme@1.0.0")).toBe("@example/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/example/acme.git"
    const expected = win ? "acme@git+https_//github.com/example/acme.git" : spec
