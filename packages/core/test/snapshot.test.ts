...
describe("Snapshot", () => {
  ...
  testEffect(Layer.empty).live("reads content fingerprints from a captured tree", () =>
    Effect.acquireUseRelease(
      Effect.promise(() => tmpdir()),
      (tmp) =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            expect(
              yield* snapshot.fingerprints({
                snapshot: before,
                paths: [RelativePath.make("tracked.txt"), RelativePath.make("missing.txt")],
              }),
            ).toEqual([
              { path: RelativePath.make("tracked.txt"), digest: Hash.sha256(Buffer.from("one\n")) },
              { path: RelativePath.make("missing.txt"), digest: "missing" },
            ])
...
async function initGit(directory: string, commit = false) {
  ...
  if (!commit) return
  await $`git -c user.email=test@ycoding.test -c user.name=Test commit --no-gpg-sign -m initial`
    .cwd(directory)
    .quiet()
}
