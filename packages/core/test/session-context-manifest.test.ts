...
function evidence(input: {
  ...
  readonly recentTail?: ReadonlyArray<ContextManifest.TargetSelector>
...
describe("ContextManifest safety and lowering checks", () => {
  ...
  test("rejects recent-tail, protected, post-boundary, and changed protected-state targets", () => {
    const fixture = duplicateFixture()
    expect(
      issueCodes(
        ContextManifest.validate({
          candidate: fixture.candidate,
          evidence: { ...fixture.evidence, recentTail: [fixture.target] },
        }),
      ),
    ).toContain("protected_recent_tail")
    expect(
      issueCodes(
        ContextManifest.validate({
          candidate: fixture.candidate,
