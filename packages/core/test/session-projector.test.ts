...
describe("SessionProjector", () => {
  ...
  it.effect("loads current revert storage", () =>
    Effect.gen(function* () {
      const db = (yield* Database.Service).db
      yield* db
        .insert(ProjectTable)
      ...
      const current = JSON.stringify({
        ...
        snapshot: "tree",
        files: [{ file: "src/current.ts", status: "modified", additions: 1, deletions: 0, patch: "@@" }],
      })
      yield* db.run(sql`update session set revert = ${current} where id = ${sessionID}`)
      ...
      expect(storedRevert?.files).toEqual([
        { file: "src/current.ts", status: "modified", additions: 1, deletions: 0, patch: "@@" },
      ])
    }),
  )
