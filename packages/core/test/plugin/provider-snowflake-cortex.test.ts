...
describe("SnowflakeCortexPlugin", () => {
  it.effect("is registered in ProviderPlugins before OpenAICompatiblePlugin", () =>
    Effect.sync(() => {
      expect(ProviderPlugins.map((item) => item.id)).toContain("ycoding.provider.snowflake-cortex")
      const ids = ProviderPlugins.map((p) => p.id)
      expect(ids.indexOf("ycoding.provider.snowflake-cortex")).toBeLessThan(
        ids.indexOf("ycoding.provider.openai-compatible"),
      )
    }),
  )
