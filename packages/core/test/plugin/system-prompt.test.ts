...
describe("SystemPromptPlugin", () => {
  ...
  test("uses granular IDs with a common prefix", () => {
    expect(SystemPromptPlugin.Plugins.map((plugin) => plugin.id)).toEqual([
      "ycoding.system-prompt.openai",
      "ycoding.system-prompt.google",
      "ycoding.system-prompt.anthropic",
      "ycoding.system-prompt.kimi",
      "ycoding.system-prompt.arcee",
      "ycoding.system-prompt.meta",
    ])
  })
  ...
  it.effect("selects model-lab prompts through session context hooks", () =>
    Effect.gen(function* () {
      ...
      const cases = [
        ["gpt-5", "You are YCoding, You and the user share the same workspace"],
        ["gpt-4.1", "You are YCoding, You and the user share the same workspace"],
        ["o3", "You are YCoding, You and the user share the same workspace"],
        ...
        ["meta/muse-spark-1.1", "YCoding powered by Meta Muse Spark"],
        ["llama-3.3", "You are ycoding, an interactive CLI tool"],
      ] as const
  ...
  it.effect("selects against the catalog model ID instead of its alias", () =>
    Effect.gen(function* () {
      ...
      expect(physicalOpenAI.system[0]?.text).toContain("You are YCoding, You and the user share the same workspace")
