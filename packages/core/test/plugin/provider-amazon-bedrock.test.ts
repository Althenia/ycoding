...
import { AmazonBedrockPlugin } from "@ycoding-ai/core/plugin/provider/amazon-bedrock"
...
describe("AmazonBedrockPlugin", () => {
  ...
  it.effect("serializes cache points through the installed Bedrock adapter", () =>
    withEnv({ AWS_ACCESS_KEY_ID: undefined, AWS_BEARER_TOKEN_BEDROCK: undefined, AWS_PROFILE: undefined }, () =>
      Effect.gen(function* () {
        const aisdk = yield* AISDK.Service
        ...
        yield* Effect.tryPromise(() =>
          result.sdk.languageModel("anthropic.claude-sonnet-4-5").doGenerate({
            prompt: [
              {
                role: "system",
                content: "stable",
        ...
        expect(body).toMatchObject({
          system: expect.arrayContaining([{ cachePoint: { type: "default", ttl: "1h" } }]),
        })
      }),
    ),
  )
