...
describe("resource", () => {
  ...
  test("drops OTEL resource attributes when any entry is invalid", () => {
    ...
    expect(resource().attributes["ycoding.client"]).toBeDefined()
  })

  test("keeps built-in attributes when env values conflict", () => {
    process.env.OTEL_RESOURCE_ATTRIBUTES =
      "ycoding.client=web,service.instance.id=override,service.namespace=anomalyco"
    ...
    expect(resource("cli").attributes["ycoding.run"]).toMatch(/^[0-9a-f]{8}$/)
  })
})

test("falls back to local logging when OTLP initialization fails", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ycoding-observability-test-"))
  await using _ = {
...
test("file logger appends concurrent runs with a run on every line", async () => {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "ycoding-log-test-"))
  await using _ = {
    async [Symbol.asyncDispose]() {
      await fs.rm(dir, { recursive: true, force: true })
    },
