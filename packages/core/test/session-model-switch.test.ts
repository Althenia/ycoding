...
describe("SessionV2.switchModel context validation", () => {
  ...
  it.effect("uses the default compaction safety margin without an advisory boundary", () =>
    Effect.gen(function* () {
      const { sessionID, location } = createSession()
      const session = yield* SessionV2.Service
      yield* session.create({ id: sessionID, location, model: sonnet })
      ...
      expect(outcome.targetSafeInputTokens).toBe(64_000 - 30_000)
  ...
  it.effect("retains the default safety margin when compaction advice is disabled", () =>
    Effect.gen(function* () {
      ...
      expect(outcome).toMatchObject({
        status: "blocked",
        targetSafeInputTokens: 64_000 - 30_000,
      })
    }),
  )
...
describe("SessionRunnerCache prompt cache namespace identity", () => {
  ...
  test("stays stable when the model id changes without changing the visible prefix", () => {
  ...
  test("stays stable when the provider changes without changing the visible prefix", () => {
    expect(SessionRunnerCache.promptCacheNamespace(base)).toBe(
      SessionRunnerCache.promptCacheNamespace({ ...base, providerID: "anthropic", modelID: "claude-sonnet-4-5" }),
    )
  })
  test("stays stable when the reasoning variant changes on one catalog model", () => {
    const high = { ...base, variant: "high" }

    expect(SessionRunnerCache.promptCacheNamespace(base)).toBe(SessionRunnerCache.promptCacheNamespace(high))
  })
