...
const AUTO_BACKGROUND_STEER =
  "The foreground shell exceeded 300 seconds and was moved to the background. Continue without waiting; completion will be delivered automatically."
              ...
              Effect.gen(function* () {
                ...
                const reservation = yield* guardrail
                  .assert({
                    ...
                    action: name,
                    resources: [input.command],
                    metadata: { workdir: target.canonical },
                  })
                  .pipe(
                    Effect.mapError(
                      (error) =>
                        new ToolFailure({
                          message: `Session guardrail rejected shell command: ${input.command}`,
                          error,
                        }),
                    ),
                  )
                ...
                const job = yield* runtime.job
                  ...
                  .pipe(
                    Effect.onError(() =>
                      shell.remove(info.id).pipe(Effect.ignore, Effect.andThen(reservation.release)),
                    ),
                  )
                ...
                const result = yield* runtime.job
                  .block({
                    id: job.id,
                    sessionID: context.sessionID,
                    ...(timeout > 0 && timeout <= AUTO_BACKGROUND_MS ? {} : { backgroundAfter: AUTO_BACKGROUND_MS }),
                  })
                  .pipe(
