...
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.YCODING_WEBSEARCH_PROVIDER === "exa" || process.env.YCODING_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.YCODING_WEBSEARCH_PROVIDER
        : undefined,
    enableExa:
      ["1", "true"].includes(process.env.YCODING_EXPERIMENTAL?.toLowerCase() ?? "") ||
      ["1", "true"].includes(process.env.YCODING_ENABLE_EXA?.toLowerCase() ?? "") ||
      ["1", "true"].includes(process.env.YCODING_EXPERIMENTAL_EXA?.toLowerCase() ?? ""),
    enableParallel:
      ["1", "true"].includes(process.env.YCODING_ENABLE_PARALLEL?.toLowerCase() ?? "") ||
      ["1", "true"].includes(process.env.YCODING_EXPERIMENTAL_PARALLEL?.toLowerCase() ?? ""),
