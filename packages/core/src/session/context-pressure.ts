...
export type AdvisoryPressure = {
  readonly level: "consider" | "advised"
...
export const level = (
  ...
  const contextLimit = input.capabilities.contextWindowTokens
  if (estimate * 100 >= contextLimit * resolved.advisory.stronglyAdvisedPercent) return "advised"
  if (estimate * 100 >= contextLimit * resolved.advisory.considerPercent) return "consider"
  return "normal"
}
...
/** Re-admit unchanged soft pressure only after it consumes another five percent of the safe input budget. */
export const shouldAdmitAdvisory = (input: {
  readonly current: AdvisoryPressure["level"]
  readonly previous: AdvisoryPressure | undefined
  readonly estimatedInputTokens: number
  readonly hardInputCapTokens: number
}) => {
  if (input.previous === undefined) return true
  if (pressureRank(input.current) > pressureRank(input.previous.level)) return true
  if (pressureRank(input.current) < pressureRank(input.previous.level)) return false
  return input.estimatedInputTokens - input.previous.estimatedInputTokens >= input.hardInputCapTokens * 0.05
}

function pressureRank(level: Level) {
