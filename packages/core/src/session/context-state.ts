...
export type ProviderCompaction = ContextManifest.ProviderCompaction & {
  readonly coveredThrough: ContextManifest.CoveredThrough
  readonly manifestDigest: string
  readonly timeActivated: number
}
...
export function selectEntries<Entry extends { readonly seq?: number; readonly message: SessionMessage.Info }>(
  ...
  return Effect.gen(function* () {
    ...
    const context = yield* loadModelContext(db, state)
    const active = context
      ? entries.filter((entry) => entry.seq === undefined || entry.seq > context.value.coveredThrough.seq)
      : entries
    ...
    if (exclusions.length === 0)
      return {
        entries: active,
        ...(context?.type === "summary" ? { summary: context.value } : {}),
        ...(context?.type === "provider" ? { providerCompaction: context.value } : {}),
      }
...
const loadModelContext = Effect.fnUntraced(function* (
  db: DatabaseService,
  ...
  const providerCompaction = Schema.decodeUnknownOption(ContextManifest.ProviderCompaction)(blob.content.providerCompaction)
  if (
    Option.isNone(providerCompaction) ||
    state.covered_through_message_id === null ||
    state.covered_through_seq === null
  )
    return undefined
  return {
    type: "provider" as const,
    value: {
      ...providerCompaction.value,
      coveredThrough: {
        messageID: state.covered_through_message_id,
        seq: EventV2.Seq.make(state.covered_through_seq),
      },
      manifestDigest: state.manifest_digest,
      timeActivated: state.time_activated,
    } satisfies ProviderCompaction,
  }
})
...
const commitActivation = Effect.fnUntraced(function* (
  ...
  if (activation.input.manifest.providerCompaction)
    yield* SessionProviderState.captureInTransaction(db, {
      ...
      partOrdinal: 0,
      partKind: "compaction",
      provider: activation.input.manifest.providerCompaction.provider,
      modelID: activation.input.manifest.providerCompaction.modelID,
      contextRevision: activation.revision,
      state: { opaqueCompactionOutput: activation.input.manifest.providerCompaction.output },
    })
...
const requireManifest = Effect.fnUntraced(function* (
  ...
  return yield* Effect.try({
    try: () => {
      ...
      if (
        manifest.providerCompaction &&
        (!Schema.is(ContextManifest.ProviderCompaction)(manifest.providerCompaction) || manifest.summary)
      )
        throw new ActivationError({ code: "invalid_manifest", message: "Manifest provider compaction is invalid" })
...
function manifestMutation(manifest: ContextManifest.Manifest) {
  ...
  if (manifest.providerCompaction && !isDeeplyFrozen(manifest.providerCompaction))
    return new ActivationError({
      code: "selector_mutation",
      message: "The provider compaction output changed after manifest validation",
    })
