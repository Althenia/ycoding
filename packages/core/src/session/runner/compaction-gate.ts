...
export class RequiredCompactionFailed extends Data.TaggedError("SessionCompactionGate.RequiredCompactionFailed")<{
...
export interface Result<Context, Prepared extends { readonly request: LLMRequest }>
  ...
  readonly requiredFailure?: RequiredCompactionFailed
}
...
> =>
  Effect.gen(function* () {
    ...
    if (cap <= 0)
      return input.force
        ? requiredFailure(input.candidate, false, "Mandatory compaction requires a positive context limit")
        : { ...input.candidate, compacted: false }
    if (!input.force && !input.refreshLatest && estimate(input.candidate) < cap)
    ...
    const gated = yield* jobs
          ...
          while (true) {
            if (gateOwnedAdmissions === 2) {
              ...
              return requiredFailure(
                rebuilt,
                compacted,
                "Mandatory compaction did not reduce the request below the context limit",
              )
            }
            const boundary = yield* latestCompleteBoundary(db, input.sessionID)
            if (!boundary)
              return input.force
                ? requiredFailure(input.candidate, compacted, "Mandatory compaction has no complete message boundary")
                : { ...input.candidate, compacted }
            const state = yield* currentState(db, input.sessionID)
            if (state === undefined)
              return input.force
                ? requiredFailure(input.candidate, compacted, "Mandatory compaction has no context revision")
                : { ...input.candidate, compacted }
            if (input.refreshLatest && state.coveredThroughSeq !== null && state.coveredThroughSeq >= boundary.seq) {
              const rebuilt = yield* input.reload({ fullRebase: true })
            ...
            if (existing) {
              ...
              if (settled.status === "failed")
                return requiredFailure(
                  rebuilt,
                  compacted,
                  `Mandatory compaction failed${settled.failure ? ` (${settled.failure})` : ""}`,
                )
              if (!input.refreshLatest && estimate(rebuilt) < cap) return { ...rebuilt, compacted }
              continue
            }
            if (
              ...
              return requiredFailure(
                input.candidate,
                compacted,
                "Mandatory compaction has an unchanged deterministic failure for this context limit",
              )
            ...
            if (input.refreshLatest) return { ...rebuilt, compacted }
            if (estimate(rebuilt) < cap) return { ...rebuilt, compacted }
          }
        }),
      )
      .pipe(
        Effect.catchIf(
          ...
          (error) =>
            Effect.succeed(
              requiredFailure(input.candidate, false, `Mandatory compaction coordination failed: ${error.message}`),
            ),
        ),
      )
    return gated
...
const requiredFailure = <Context, Prepared extends { readonly request: LLMRequest }>(
  candidate: Candidate<Context, Prepared>,
  compacted: boolean,
  message: string,
): Result<Context, Prepared> => ({
  ...candidate,
  compacted,
  requiredFailure: new RequiredCompactionFailed({ message }),
})
...
const currentState = Effect.fnUntraced(function* (db: Database.Interface["db"], sessionID: SessionSchema.ID) {
  const state = yield* db
    .select({
      revision: SessionContextStateTable.revision,
      coveredThroughSeq: SessionContextStateTable.covered_through_seq,
    })
