...
export const layer = (options: Options = {}) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const generation: Interface["generation"] = () => Effect.succeed(0)

      return Service.of({ policy, observe, generation })
    }),
  )
...
function measuredCacheHitRatio(row: {
  readonly invalidation: string
  readonly cacheReadReported: boolean | null
  readonly tokens: { readonly input: number; readonly cache: { readonly read: number; readonly write: number } }
}, minimumTokens: number | undefined): number | undefined {
  if (
    row.invalidation === "below-minimum" ||
    row.invalidation === "cache-disabled" ||
    row.invalidation === "retry-fallback"
  )
    return undefined
  if (row.cacheReadReported !== true) return undefined
  const eligible = row.tokens.input + row.tokens.cache.read + row.tokens.cache.write
  if (eligible <= 0) return undefined
  if (minimumTokens !== undefined && eligible < minimumTokens) return undefined
  return row.tokens.cache.read / eligible
}
