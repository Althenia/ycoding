...
export function parse(input: string, options: ParseOptions): Memory | ParseError {
  ...
  if (!isRecord(body)) return new InvalidElementError({ reason: "conversation_memory must be an object" })
...
function rootBody(value: unknown): { body: unknown } | { reason: string } {
  if (!isRecord(value)) return { reason: "not an object document" }
