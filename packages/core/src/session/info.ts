...
import { Revert } from "@ycoding-ai/schema/session-revert"
import { Money } from "@ycoding-ai/schema/money"

const decodeRevert = Schema.decodeUnknownSync(Revert)

export function fromRow(row: typeof SessionTable.$inferSelect): SessionSchema.Info {
  const permissionCeiling = SessionPermissionCeiling.read(row.permission)
  return SessionSchema.Info.make({
    ...
    time: {
      ...
      pinned: row.time_pinned === null ? undefined : DateTime.makeUnsafe(row.time_pinned),
    },
  })
}
