...
export const activeKeys = Effect.fn("InstructionState.activeKeys")(function* (
  db: DatabaseService,
  instructions: Instructions.Instructions,
  sessionID: SessionSchema.ID,
) {
  const [observed, state] = yield* Effect.all([Instructions.read(instructions), readState(db, sessionID)], {
    concurrency: "unbounded",
  })
  const current = yield* observeAgainst(
    state ? observed : observed.filter((entry) => entry.value !== Instructions.unavailable),
    state?.current_values,
  ).pipe(Effect.orDie)
  return Object.keys(current.current).map((key) => Instructions.Key.make(key))
})
