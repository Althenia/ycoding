...
export type RuntimeEvidence = {
  ...
  readonly recentTail: ReadonlyArray<TargetSelector>
...
export function validate(input: ValidationInput): ValidationResult {
  ...
  const recentTail = new Set(input.evidence.recentTail.map(selectorIdentity))
  const protectedTargets = new Set(input.evidence.protectedTargets.map(selectorIdentity))
  const summarized = validateSummary(input.summary, input.evidence, indexed, recentTail, protectedTargets, issues)
  const targetIdentities = new Set(summarized.items.map(itemIdentity))
  const resolved = input.candidate.exclusions.flatMap((exclusion) => {
    ...
    if (recentTail.has(identity))
      addIssue(issues, "protected_recent_tail", `Target ${identity} belongs to the protected recent tail`)
  ...
  if (excludedItems.size >= input.evidence.items.length && summarized.summary === undefined)
...
function validateSummary(
  ...
  items.forEach((item) => {
    ...
    if (recentTail.has(identity))
      addIssue(issues, "protected_recent_tail", `Summary target ${identity} belongs to the protected recent tail`)
    if (protectedTargets.has(identity))
