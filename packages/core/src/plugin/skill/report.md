...
# Report a YCoding Issue

Use this skill when the user wants to report a YCoding issue or bug. Your job
...
- YCoding version: run `ycoding --version`.
...
- Active plugins: inspect YCoding config for configured plugins when possible.
  Check likely config locations such as `ycoding.json`, `ycoding.jsonc`,
  `.ycoding/ycoding.json`, and `~/.config/ycoding/ycoding.json`. Record
  configured plugin entries, local plugin files under `.ycoding/plugin/` or
  `.ycoding/plugins/`, and note if plugin status could not be determined.
...
- Recent changes that may be related, such as updating YCoding, changing
...
- YCoding version: <!-- value or Unavailable: reason -->
