...
export interface Interface {
  ...
  readonly tree: {
    ...
    readonly read: (input: {
      ...
      tree: TreeID
      file: RelativePath
    }) => Effect.Effect<Uint8Array | undefined, OperationError>
    readonly diff: (input: {
      repository: Repository
      from: TreeID
      to: TreeID
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const repositoryOperation = Effect.fnUntraced(function* (
      ...
      if (result.exitCode === 0) return { text, stdout: result.stdout, stderr: result.stderr.toString("utf8") }
      return yield* new OperationError({
        operation: operationName,
        directory: repository.worktree,
    ...
    const treeRead = Effect.fn("Git.tree.read")(function* (input: {
      ...
      const source = yield* entry(input.repository, input.tree, input.file)
      if (!source) return undefined
      return (yield* repositoryOperation("read_tree", input.repository, ["cat-file", "blob", source.object])).stdout
    })
