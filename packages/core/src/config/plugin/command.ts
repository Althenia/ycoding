        ...
        if (entry.type === "document") {
          ...
          const locations =
            entry.path && path.isAbsolute(entry.path)
              ? Object.fromEntries(
                  Object.keys(commands ?? {}).map((name) => [name, [AbsolutePath.make(entry.path!)]]),
                )
              : {}
          return Effect.succeed([{ commands, locations }])
        }
          ...
          Effect.map((commands) => [
            {
              ...
              locations: Object.fromEntries(commands.map((command) => [command.name, [command.filepath]])),
            },
          ]),
        )
      ...
      for (const document of loaded.documents) {
        for (const [name, command] of Object.entries(document.commands ?? {})) {
          draft.update(name, (item) => {
            ...
            for (const location of document.locations[name] ?? []) {
              if (!item.locations?.includes(location)) item.locations = [...(item.locations ?? []), location]
            }
...
function loadDirectory(fs: FSUtil.Interface, directory: string) {
  return Effect.gen(function* () {
    ...
    return yield* Effect.forEach(files.toSorted(), (filepath) =>
      ...
      Effect.map((commands) =>
        commands.filter(
          (command): command is { name: string; info: ConfigCommand.Info; filepath: AbsolutePath } =>
            command !== undefined,
        ),
      ),
    )
...
function decode(directory: string, filepath: string, content: string) {
  ...
  if (!info) return
  return {
    filepath: AbsolutePath.make(filepath),
    name: path
