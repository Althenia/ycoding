...
describe("contract hygiene", () => {
  ...
  test("provider cache telemetry is durable while the legacy cache mechanism remains absent", () => {
    ...
    expect(SessionEvent.Step.Ended.data.fields).toHaveProperty("providerCache")
    expect(SessionEvent.Step.Failed.data.fields).toHaveProperty("providerCache")
    expect(
  ...
  test("keeps revert state current-only", () => {
    ...
    expect(String(revert.messageID)).toBe("msg_current")
    ...
    expect(revert).not.toHaveProperty("partID")
    expect(revert).not.toHaveProperty("diff")
    expect(Schema.encodeSync(Session.Revert)(revert)).toEqual(revert)
  })
})
