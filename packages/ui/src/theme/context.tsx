...
const STORAGE_KEYS = {
  THEME_ID: "ycoding-theme-id",
  COLOR_SCHEME: "ycoding-color-scheme",
  THEME_CSS_LIGHT: "ycoding-theme-css-light",
  THEME_CSS_DARK: "ycoding-theme-css-dark",
} as const

const THEME_STYLE_ID = "oc-theme"
let files: Record<string, () => Promise<{ default: ThemeDefinition }>> | undefined
let ids: string[] | undefined
let known: Set<string> | undefined

function getFiles() {
  if (files) return files
  files = import.meta.glob<{ default: ThemeDefinition }>("./themes/*.json")
  return files
}
...
const yc2Theme = yc2ThemeJson as ThemeDefinition
...
function applyThemeCss(theme: ThemeDefinition, themeId: string, mode: "light" | "dark") {
...
function cacheThemeVariants(theme: ThemeDefinition, themeId: string) {
  if (themeId === "yc-2") return
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: {
    defaultTheme?: string
    onThemeApplied?: (theme: ThemeDefinition, mode: "light" | "dark", scheme: ColorScheme) => void
  }) => {
    const themeId = normalize(read(STORAGE_KEYS.THEME_ID) ?? props.defaultTheme) ?? "yc-2"
    ...
    const loads = new Map<string, Promise<ThemeDefinition | undefined>>()
    ...
    const applyTheme = (theme: ThemeDefinition, themeId: string, mode: "light" | "dark", scheme: ColorScheme) => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === STORAGE_KEYS.THEME_ID && e.newValue) {
        ...
        if (!next) return
        if (next !== "yc-2" && !knownThemes().has(next) && !store.themes[next]) return
    ...
    onMount(() => {
      ...
      const savedTheme = normalize(rawTheme ?? props.defaultTheme) ?? "yc-2"
    ...
    const setTheme = (id: string) => {
      ...
      if (next !== "yc-2" && !knownThemes().has(next) && !store.themes[next]) {
        console.warn(`Theme "${id}" not found`)
        return
      }
      setStore("themeId", next)
    ...
    return {
      ...
      setTheme,
      setColorScheme,
      registerTheme: (theme: ThemeDefinition) => setStore("themes", theme.id, theme),
