...
const themePath = import.meta.dir + "/../src/theme/themes/yc-2.json"
const theme = (await Bun.file(themePath).json()) as ThemeDefinition
...
console.log("Updated yc-2.json v2Overrides", Object.keys(light).length, "tokens per mode")
...
function readTokens(mode: "light" | "dark") {
  ...
  if (!block) throw new Error(`Missing ${mode} YC-2 tokens`)
