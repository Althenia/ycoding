...
test("decodes ui.matches text params", () => {
  expect(
    Frontend.decodeRequest({
      ...
      id: 1,
      method: "ui.matches",
      params: { text: "YCoding [ready].*" },
    }),
  ).toMatchObject({ method: "ui.matches", params: { text: "YCoding [ready].*" } })
  expect(() =>
    Frontend.decodeRequest({
      jsonrpc: "2.0",
      id: 1,
      method: "ui.matches",
      params: { pattern: "YCoding.*" },
    }),
  ).toThrow()
})
...
test("decodes semantic UI snapshots", () => {
  ...
  for (const nodes of [
    ...
    expect(() => decode({ format: "ycoding-ui-snapshot-v1", nodes })).toThrow()
})
...
const params: Handshake.Params = {
  client: { name: "ycoding-drive", version: "test" },
  expectedRole: "ui",
  offeredVersions: [1],
...
const ui: Handshake.DispatchAction = {
  role: "ui",
  server: { name: "ycoding", version: "test" },
