    ...
    Effect.gen(function* () {
      ...
      expect(yield* Queue.take(messages)).toMatchObject({
        ...
        result: {
          protocolVersion: 1,
          role: "backend",
          server: { name: "ycoding", version: expect.any(String) },
...
test("controls arbitrary tools through scoped SDK overlays", async () => {
  const endpoint = availableEndpoint()
  const directory = await mkdtemp(join(tmpdir(), "ycoding-simulated-tools-"))
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        ...
        yield* plugins.register(
          Plugin.define({
            id: "ycoding.simulation.test.activation-count",
