...
test("matches literal screen text", () => {
  const harness = { screen: () => "YCoding [ready].*" }

  expect(matches(harness, "YCoding")).toBe(true)
  expect(matches(harness, "[ready].*")).toBe(true)
  expect(matches(harness, "YCoding.*ready")).toBe(false)
  expect(matches(harness, "ycoding")).toBe(false)
})
