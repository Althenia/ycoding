...
import { DEFAULT_BASE_URL, PATH } from "@ycoding-ai/ai/protocols/openai-chat"
...
test("encodes provider-neutral partial tool input as OpenAI deltas", async () => {
  const provider: SimulatedProvider.Interface = {
    stream: () =>
      Stream.make(
        ...
        { type: "toolInputDelta", index: 0, text: '"YCoding"}' },
  ...
  expect(body).toBe(
    [
      ...
      'data: {"choices":[{"delta":{"tool_calls":[{"index":0,"function":{"arguments":"\\"YCoding\\"}"}}]}}]}',
