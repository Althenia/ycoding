...
import { PositiveInt } from "@ycoding-ai/core/schema"
...
export const resolve = Effect.fn("DriveManifest.resolve")(function* () {
  const name = yield* Config.schema(InstanceName, "YCODING_DRIVE").pipe(Effect.mapError(configError))
  ...
  const directory = yield* Config.string("DRIVE_REGISTRY_DIR").pipe(
    Config.withDefault(join(state, "ycoding-drive", "instances")),
    Effect.mapError(configError),
  )
