...
export class Service extends Context.Service<Service, Interface>()("@ycoding/simulation/SimulatedProvider") {}
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      yield* SimulationControlServer.start({
        ...
        label: "ycoding drive backend websocket",
        data: () => ({}),
      ...
      yield* Effect.sync(() => process.stderr.write(`ycoding drive backend websocket: ${options.endpoint}\n`))
  ...
  switch (request.method) {
    case "simulation.handshake":
      return SimulationProtocol.Handshake.dispatch(
        {
          role: "backend",
          server: { name: "ycoding", version: InstallationVersion },
