...
export const create = Effect.fn("Drive.create")(function* (options: CliRendererConfig) {
  const headless = (yield* Config.string("YCODING_DRIVE_RENDERER").pipe(Config.withDefault("visible"))) === "headless"
  ...
  yield* Effect.sync(() => process.stderr.write(`ycoding drive ui websocket: ${server.url}\n`))
  return renderer
})
