...
import type { TuiAttention } from "./attention.js"
...
export interface Data {
  readonly on: <Type extends YCodingEvent["type"]>(
    type: Type,
    handler: (event: Extract<YCodingEvent, { type: Type }>) => void,
  ) => () => void
  readonly listen: (handler: (event: { details: YCodingEvent }) => void) => () => void
  readonly session: {
    list(): SessionInfo[]
...
export interface KeymapLayer {
  /** Limits the layer to one YCoding input mode. Use global to opt out; defaults to base. */
...
export interface Keymap {
  ...
  /** Controls mutually exclusive YCoding input modes. */
  readonly mode: {
