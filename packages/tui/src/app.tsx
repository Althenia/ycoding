...
import { registerYCodingSpinner } from "./component/register-spinner"
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
      ...
      const renderer = yield* Effect.gen(function* () {
        ...
        if (process.env.YCODING_DRIVE) {
          const { Drive } = yield* Effect.promise(() => import("@ycoding-ai/simulation/frontend"))
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          <TuiStartupProvider
                            value={{
                              initialRoute: process.env.YCODING_SCRAP
                                ? { type: "plugin", id: "scrap", name: "scrap" }
                                : process.env.YCODING_ROUTE
                                  ? JSON.parse(process.env.YCODING_ROUTE)
                                  : undefined,
                              skipInitialLoading: Boolean(process.env.YCODING_FAST_BOOT),
                            }}
                          >
                            <ClipboardProvider>
...
function App(props: { pair?: DialogPairCredentials; started: number }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle("YCoding")
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`YC | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`YC | ${route.data.name}`)
    }
  })
