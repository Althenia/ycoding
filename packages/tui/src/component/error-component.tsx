        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            <text fg={colors.muted}>
              {copied()
                ? "Report copied."
                : "Copy the report and attach it to a private security advisory or issue."}
            </text>
            <text fg={colors.muted}>YCoding {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
export function buildCrashReport(message: string, stack: string) {
  return [
    ...
    stack,
    "",
    "Describe what you were doing immediately before the crash.",
  ].join("\n")
}
