  ...
  init: (props: { mode: "dark" | "light"; source?: ThemeSource }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme?.name ?? "ycoding"
        draft.active = typeof active === "string" ? active : "ycoding"
        draft.ready = false
      }),
    )
    ...
    function syncCustomThemes() {
      return themes
        ...
        .then((themes) => {
          setCustomThemes(
            Object.entries(themes).reduce<Record<string, ThemeFile>>((result, [name, theme]) => {
              if (isTheme(theme)) result[name] = theme
              return result
            }, {}),
          )
        })
        .catch(() => setStore("active", "ycoding"))
    }
      ...
      return renderer
        ...
        .then((colors: TerminalColors) => {
          if (!colors.palette[0]) {
            ...
            if (store.active === "system") setStore("active", "ycoding")
            return
          }
    ...
    const source = createMemo(() => store.themes[store.active] ?? store.themes.ycoding!)
    const sourceName = createMemo(() => (store.themes[store.active] ? store.active : "ycoding"))
    const file = source
