...
export type SubagentTracker = {
  ...
  ready(): Promise<void>
  select(sdk: YCodingClient, sessionID: string | undefined): void
...
export function createSubagentTracker(input: SubagentTrackerInput): SubagentTracker {
  ...
  // Child session launched by each live subagent tool call, so a call that ends
  // without a structured payload (tool.failed) can still settle its child.
  ...
  // Drop one tracked child and every per-child buffer keyed by its session, so a
  // shed entry cannot leak timers or replay events into a later re-adoption.
  const release = (child: ChildState) => {
    children.delete(child.sessionID)
    pendingEvents.delete(child.sessionID)
  ...
  // Family cap policy: the newest subagent always wins a slot. Only settled
  // children are shed, oldest-settled first, and never the selected one whose
  // transcript is on screen. When every entry is still running the list is
  // allowed to grow past the cap: dropping a live child would blank its tab
  // while its work continues, which is worse than briefly exceeding the bound.
  const sheddable = () => {
    let victim: ChildState | undefined
    for (const child of children.values()) {
      if (child.status === "running" || child.sessionID === selected) continue
      if (!victim || child.lastUpdatedAt < victim.lastUpdatedAt) victim = child
    }
    return victim
  }

  const evictForAdmission = () => {
    while (children.size >= FAMILY_LIST_LIMIT) {
      const victim = sheddable()
      if (!victim) return
      release(victim)
    }
  }

  const admitChild = (sessionID: string): ChildState => {
    const existing = children.get(sessionID)
    if (!existing) evictForAdmission()
  ...
  const hydrateChild = (sdk: YCodingClient, child: ChildState, signal = input.signal): Promise<void> => {
  ...
  function scheduleBlockerRetry(sdk: YCodingClient, child: ChildState, epoch: number, signal = input.signal) {
  ...
  const discover = (sdk: YCodingClient, sessionID: string, signal = input.signal) => {
    // At the cap, a lookup only starts when admission could still win a slot,
    // so an indirectly discovered child sheds a settled entry exactly like a
    // directly launched one. When nothing is sheddable the lookup is skipped
    // rather than growing the list: the direct path grows because this session
    // launched that child, while discovery is driven by foreign session traffic
    // this session does not control, so growing here has no bound at all.
    if (!active(signal) || (children.size >= FAMILY_LIST_LIMIT && !sheddable())) return Promise.resolve()
  ...
  const mainTool = (
    ...
    next: {
      ...
      reconnect?: boolean
      // Children of subagent calls this pass saw still in flight.
      blocking?: Set<string>
    },
  ) => {
    ...
    if (child.status === "running") {
      // A subagent call that has not returned still blocks the parent, so its
      // child is live no matter what the active snapshot says. Once the call
      // returned, the structured status plus the active map decide, which is how
      // a backgrounded child stays running. Children already tracked from the
      // live stream keep their streamed status unless this is a reconnect, where
      // a settled event can be missed while the stream is down.
      const streamed = next.reconnect !== true && next.live?.has(found.sessionID) === true
      const blocking = tool.state.status === "running"
      if (blocking) next.blocking?.add(found.sessionID)
      const running = streamed || blocking || (found.running && (!next.active || found.sessionID in next.active))
      child.status = running ? "running" : "completed"
    }
    touch(child, tool.time.completed ?? tool.time.created)
  }

  // Settle a still-running child from a parent-side signal. The child's own
  // settled event stays authoritative: anything already settled is left alone.
  const settle = (child: ChildState, status: FooterSubagentTab["status"], created?: number) => {
  ...
  const settleCall = (key: string, status: FooterSubagentTab["status"], created?: number) => {
    const sessionID = callChildren.get(key)
    if (!sessionID) return
    callChildren.delete(key)
    const child = children.get(sessionID)
    if (!child || child.background) return
    if (settle(child, status, created)) input.emit()
  }

  // Children of unfinished foreground subagent calls in the parent. Backgrounded
  // children left `callChildren` when their call returned, and descendants of a
  // backgrounded child were never tracked there, so neither is settled here.
  const settleForeground = (status: FooterSubagentTab["status"], created?: number) => {
    let changed = false
    for (const sessionID of callChildren.values()) {
      const child = children.get(sessionID)
      if (!child || child.background) continue
      changed = settle(child, status, created) || changed
    }
    callChildren.clear()
    if (changed) input.emit()
  }
  ...
  return {
    main(sdk, event, signal = input.signal) {
      ...
      if (event.type === "session.execution.succeeded" || event.type === "session.execution.interrupted") {
        // A foreground subagent blocks the parent's execution, so once the parent
        // settles no foreground child can still be running. Its own settled event
        // is the normal signal; this closes the gap when that event is missed.
        settleForeground(event.type === "session.execution.interrupted" ? "cancelled" : "completed", event.created)
        return
      }
      if (event.type === "session.tool.failed") {
        const key = sourceKey(event.data.assistantMessageID, event.data.callID)
        pendingCalls.delete(key)
        // The subagent tool only fails once its child failed or was cancelled, so
        // settle the child the call launched even if its own event never arrived.
        settleCall(key, "error", event.created)
        return
      }
      ...
      if (!found) return
      const child = admitChild(found.sessionID)
      if (event.type === "session.tool.progress") callChildren.set(key, found.sessionID)
    ...
    async hydrate(next) {
      ...
      if (!active(signal)) return
      // Children already tracked from the live event stream before this pass.
      const live = new Set(children.keys())
      // Children whose subagent call the server still reports as in flight.
      ...
      for (const message of next.messages) {
        if (message.type !== "assistant") continue
        for (const item of message.content) {
          if (item.type === "tool") mainTool(item, { active: next.active, live, blocking, reconnect: next.reconnect })
        }
      }
      ...
      // A hard cap rather than the shed-a-settled-entry policy `discover` uses:
      // every child adopted above is provisionally running until the settle pass
      // below, so nothing here can tell a settled entry from a live one. A live
      // child missed at the cap is still adopted from its own events later.
      ...
      if (!active(signal)) return
      for (const child of children.values()) {
        // The active map is newer than tracker state only for children this pass
        // adopted (projected tool output, family list, active discovery) and for
        // reconnects, where a settled event can be missed while the stream is
        // down. For children already tracked across an in-place hydrate (resize
        // replay) it is older than live state -- their events were buffered, not
        // lost -- so it must not settle them. A child whose subagent call the
        // server still reports as in flight is live under either reading.
        if (
          child.status === "running" &&
          !(child.sessionID in next.active) &&
          !blocking.has(child.sessionID) &&
          (next.reconnect === true || !live.has(child.sessionID))
        )
          child.status = "completed"
