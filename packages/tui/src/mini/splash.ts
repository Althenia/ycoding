...
//
// Renders the YCoding entry wordmark and a compact YC exit mark, plus
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    const mark = go.left.slice(1).map((line, index) => `${line} ${go.right[index + 1] ?? ""}`)
    const top = 1
    ...
    push(lines, body_left, top, "YCoding", right, undefined, TextAttributes.BOLD)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `ycoding mini -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
