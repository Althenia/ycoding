// Per-tool display rules shared across `ycoding run` output paths.
//
...
import type { SessionMessageAssistantTool } from "@ycoding-ai/client/promise"
...
function runSkill(p: ToolProps): ToolInline {
  const name = p.input.name ?? p.input.id ?? ""
  return {
    icon: p.frame.status === "completed" ? "✦" : "→",
    title: `Skill "${name}"${p.frame.status === "completed" ? "  Loaded" : ""}`,
  }
}
