...
import type { FormInfo } from "@ycoding-ai/client/promise"
...
function provider() {
  return {
    id: "opencode", // YCODING_EXTERNAL_OPENCODE: external provider fixture
    name: "OpenCode", // YCODING_EXTERNAL_OPENCODE: external provider fixture
    models: {
...
test("selectedCommand backfills the catalog source for bound drafts", () => {
  const catalog = [command({ name: "ycoding-ts", description: "TS skill", source: "skill" })]

  // The skill picker binds `/name ` drafts; older drafts may lack source.
  expect(selectedCommand("/ycoding-ts fix it", { name: "ycoding-ts", arguments: "" }, catalog)).toEqual({
    name: "ycoding-ts",
    arguments: "fix it",
    source: "skill",
  })
  // An explicit source wins without a catalog lookup.
  expect(selectedCommand("/ycoding-ts", { name: "ycoding-ts", arguments: "", source: "skill" })).toEqual({
    name: "ycoding-ts",
    arguments: "",
    source: "skill",
  })
...
test("direct model panel renders current model selector", async () => {
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("OpenCode") // YCODING_EXTERNAL_OPENCODE
    expect(frame).toContain("GPT-5")
