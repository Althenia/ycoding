import { describe, expect, test } from "bun:test"
import {
  collapseSection,
  defaultExpanded,
  expandSection,
  railPlacement,
  railWidth,
  resolveExpanded,
  type RailSectionKey,
} from "../src/routes/session/rail"

describe("rail default expansion", () => {
  test("expands only session context and todo by default", () => {
    expect(defaultExpanded({})).toEqual([
      "session",
      "context",
      "todo",
    ])
  })

  test("keeps goal and autonomy collapsed alongside the persistent sections", () => {
    expect(defaultExpanded({ goal: true, autonomy: true })).toEqual([
      "session",
      "context",
      "todo",
    ])
  })

  test("keeps SHELLS collapsed while its composer surface is active", () => {
    expect(defaultExpanded({ shellSurface: true, goal: true, autonomy: true })).toEqual([
      "session",
      "context",
      "todo",
    ])
  })

  test("keeps unrelated sections collapsed without broadening to operational ones", () => {
    expect(defaultExpanded({ allExpanded: true })).toEqual([
      "session",
      "context",
      "todo",
    ])
  })
})

describe("rail expansion capacity", () => {
  test("keeps every expanded section open", () => {
    const order: RailSectionKey[] = ["session", "context", "todo", "goal"]
    expect(expandSection(order, "mcp")).toEqual(["session", "context", "todo", "goal", "mcp"])
  })

  test("re-expanding a section refreshes its recency instead of duplicating it", () => {
    const order: RailSectionKey[] = ["session", "context", "todo", "goal"]
    expect(expandSection(order, "session")).toEqual(["context", "todo", "goal", "session"])
  })

  test("collapsing removes the section and frees capacity", () => {
    const order: RailSectionKey[] = ["session", "context", "todo", "goal"]
    expect(collapseSection(order, "todo")).toEqual(["session", "context", "goal"])
  })

  test("collapsing a section that is already collapsed changes nothing", () => {
    const order: RailSectionKey[] = ["session", "context"]
    expect(collapseSection(order, "mcp")).toEqual(order)
  })
})

describe("rail attention", () => {
  test("opens an attention section without collapsing CONTEXT", () => {
    const order: RailSectionKey[] = ["session", "context", "todo", "goal"]
    expect(resolveExpanded({ order, attention: ["subagents"] })).toEqual([
      "session",
      "context",
      "todo",
      "goal",
      "subagents",
    ])
  })

  test("leaves the order untouched when nothing needs attention", () => {
    const order: RailSectionKey[] = ["session", "context", "todo"]
    expect(resolveExpanded({ order, attention: [] })).toEqual(order)
  })
})

describe("rail placement", () => {
  test("hides the rail below 100 columns", () => {
    expect(railPlacement(80)).toBe("hidden")
    expect(railPlacement(99)).toBe("hidden")
  })

  test("overlays the rail between 100 and 119 columns", () => {
    expect(railPlacement(100)).toBe("overlay")
    expect(railPlacement(119)).toBe("overlay")
  })

  test("docks the rail from 120 columns", () => {
    expect(railPlacement(120)).toBe("docked")
    expect(railPlacement(160)).toBe("docked")
  })

  test("interpolates the docked rail from 32 to 50 columns", () => {
    expect(railWidth(120)).toBe(32)
    expect(railWidth(140)).toBe(41)
    expect(railWidth(159)).toBeCloseTo(49.55)
    expect(railWidth(160)).toBe(50)
    expect(railWidth(240)).toBe(50)
  })

  test("changes smoothly around the former design viewport widths", () => {
    expect(Math.abs(railWidth(188) - railWidth(189))).toBeLessThanOrEqual(1)
    expect(Math.abs(railWidth(189) - railWidth(190))).toBeLessThanOrEqual(1)
    expect(Math.abs(railWidth(219) - railWidth(220))).toBeLessThanOrEqual(1)
    expect(Math.abs(railWidth(220) - railWidth(221))).toBeLessThanOrEqual(1)
  })
})
