...
test("accepts r while a bracketed paste awaits attachment lookup", async () => {
  submittedPrompt = undefined
  const screen = await renderScreen({ width: 100, height: 69, args: { sessionID }, route, settle: "Message YCoding…" })
  try {
    const promptRow = screen.lines().findIndex((line) => line.includes("Message YCoding…"))
    await screen.mouse.click(3, promptRow)
    const paste = screen.input.pasteBracketedText("p")
    await screen.input.typeText("r")
    await paste
    for (let attempt = 0; attempt < 100; attempt++) {
      if (screen.lines().some((line) => line.trim() === "rp")) break
      await Bun.sleep(20)
    }
    expect(screen.lines().some((line) => line.trim() === "rp")).toBe(true)

    screen.input.pressEnter()
    await waitForFrameText(screen, "Message YCoding…")
    expect(submittedText()).toBe("rp")
  } finally {
    await screen.dispose()
  }
})
