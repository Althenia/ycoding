...
async function expectRailDesign(viewport: typeof DESIGN_VIEWPORT) {
  ...
  try {
    ...
    const rowOf = (label: string) => lines.findIndex((line) => line.slice(railStart).includes(label))
    expect(rowOf("Provider cache audit")).toBe(7)
    expect(lines[9]?.slice(railStart)).not.toContain("ses_")
    expect(rowOf("CONTEXT")).toBe(13)
    // CONTEXT renders its provider identity before the model to distinguish matching model IDs.
    expect(lines.findIndex((line) => line.slice(railStart).includes("Provider") && line.includes("anthropic"))).toBe(16)
    expect(rowOf("Model")).toBe(17)
    expect(rowOf("Context")).toBe(18)
    const cache = rowOf("Cache")
    expect(cache).toBe(19)
    expect(rowOf("SPEND")).toBe(21)
    expect(rowOf("Total")).toBe(22)
    expect(rowOf("CACHE")).toBe(24)
    expect(rowOf("Reads")).toBe(25)
    expect(rowOf("Writes")).toBe(26)
