...
function route(url: URL) {
  ...
  if (url.pathname === "/api/session") return json({ data: [parent, child], cursor: {} }); if (url.pathname === "/api/session/active") return json({ data: { [sessionID]: {} } }); if (url.pathname === `/api/session/${sessionID}`) return json({ data: child }); if (url.pathname === `/api/session/${parentID}`) return json({ data: parent })
