...
const route: FetchHandler = (url) => {
  ...
  if (url.pathname === `/api/session/${sessionID}/autonomy`)
    return json({
      data: { mode: "normal", yolo: false, goal: { runID: "run_expanded_goal_yolo_capture_fixture", text: "Fix provider cache accounting", status: "active", iteration: 3, noProgress: 3, maxNoProgress: 5 },
      },
    })
