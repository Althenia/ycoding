...
export type InputUsageSemantics = "inclusive-total" | "exclusive-input" | "ai-sdk"

export type NormalizedInputUsage = {
...
export const normalizeInputUsage = (input: {
  readonly total?: number
  readonly nonCached?: number
  readonly cacheRead?: number
  readonly cacheWrite?: number
  readonly semantics: InputUsageSemantics
}): NormalizedInputUsage => {
  const total = tokenCount(input.total)
  const suppliedNonCached = tokenCount(input.nonCached)
  const cacheRead = tokenCount(input.cacheRead)
  const cacheWrite = tokenCount(input.cacheWrite)

  if (input.semantics === "exclusive-input") {
    return {
      inputTokens: sumTokens(suppliedNonCached, cacheRead, cacheWrite),
      nonCachedInputTokens: suppliedNonCached,
      cacheReadInputTokens: cacheRead,
      cacheWriteInputTokens: cacheWrite,
    }
  }

  const inputTokens = total ?? sumTokens(suppliedNonCached, cacheRead, cacheWrite)
  const normalizedRead =
    cacheRead === undefined || inputTokens === undefined ? cacheRead : Math.min(cacheRead, inputTokens)
  const remainingAfterRead = subtractTokens(inputTokens, normalizedRead)
  const normalizedWrite =
    cacheWrite === undefined || remainingAfterRead === undefined
      ? cacheWrite
      : Math.min(cacheWrite, remainingAfterRead)
  const derivedNonCached = subtractTokens(remainingAfterRead, normalizedWrite)
  const reportedTotal = sumTokens(suppliedNonCached, normalizedRead, normalizedWrite)
  const breakdownMatches = inputTokens === undefined || reportedTotal === undefined || inputTokens === reportedTotal

  return {
    inputTokens,
    nonCachedInputTokens:
      input.semantics === "ai-sdk" && !breakdownMatches ? derivedNonCached : suppliedNonCached ?? derivedNonCached,
    cacheReadInputTokens: normalizedRead,
    cacheWriteInputTokens: normalizedWrite,
  }
}
