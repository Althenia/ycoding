...
// role. Converse rejects consecutive same-role turns with "roles must alternate
// between user and assistant", so merge them here; appending preserves block
// order and every positional cachePoint marker already emitted.
const appendUserBlocks = (messages: BedrockMessage[], content: ReadonlyArray<BedrockUserBlock>) => {
...
const lowerMessages = Effect.fn("BedrockConverse.lowerMessages")(function* (
  ...
  for (const message of request.messages) {
    if (message.role === "system") {
      ...
      appendUserBlocks(messages, textWithCache(breakpoints, part.text, part.cache))
      continue
    }
