...
const OpenAIChatSystemText = Schema.Struct({
  type: Schema.Literal("text"),
  text: Schema.String,
  prompt_cache_breakpoint: Schema.optional(OpenAIOptions.OpenAIPromptCacheBreakpoint),
})
...
const OpenAIChatMessage = Schema.Union([
  ...
  Schema.Struct({
    ...
    content: Schema.Union([Schema.String, Schema.Array(OpenAIChatSystemText)]),
  }),
  Schema.Struct({
...
export const OpenAIChatEvent = Schema.Struct({
  ...
  // OpenRouter-only: how much the response saved (or, for cache writes on
  // some providers, cost extra) from prompt caching. Native OpenAI never
  // sends this — keep it optional/nullable so its absence never fails
  // decoding. https://openrouter.ai/docs/features/prompt-caching
  cache_discount: optionalNull(Schema.Number),
})
...
const lowerUserMessage = Effect.fn("OpenAIChat.lowerUserMessage")(function* (
  ...
  // Collapsing to a plain string is only safe when no block carries a
  // breakpoint marker — a marker must live on a `text` content block.
...
const mapUsage = (event: OpenAIChatEvent): Usage | undefined => {
  ...
  // `cache_discount` rides on the event, not the nested usage object
  // (OpenRouter-only), so it is merged in here rather than declared on
  // `OpenAIChatUsage` — keeping it out of the shape shared with native OpenAI.
  return new Usage({
    ...normalized,
    ...
    providerMetadata: {
      openai:
        event.cache_discount !== undefined && event.cache_discount !== null
          ? { ...usage, cache_discount: event.cache_discount }
          : usage,
    },
  })
}
...
const step = (state: ParserState, event: OpenAIChatEvent) =>
  Effect.gen(function* () {
    ...
    const usage = mapUsage(event) ?? state.usage
