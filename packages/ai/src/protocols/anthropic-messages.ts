...
// `cache_creation_input_tokens` is the flat total; `cache_creation` is the
// same total broken down by TTL bucket. Both are decoded so downstream
// consumers can distinguish 5-minute from 1-hour cache writes without the
// per-TTL detail being silently stripped as an undeclared field.
const AnthropicCacheCreationBreakdown = Schema.Struct({
  ephemeral_5m_input_tokens: Schema.optional(Schema.Number),
...
const AnthropicUsage = Schema.Struct({
  ...
  cache_creation: optionalNull(AnthropicCacheCreationBreakdown),
})
...
const lowerTool = (breakpoints: Cache.Breakpoints, tool: ToolDefinition, inputSchema: JsonSchema): AnthropicTool => ({
  ...
  input_schema: ToolSchemaProjection.anthropic(inputSchema),
...
// Mid-conversation system messages remain model-capability gated. Unsupported
// Anthropic models use the visible wrapped-user fallback instead of a role they reject.
const supportsNativeSystemUpdates = (request: LLMRequest) =>
  AnthropicModel.capabilities(String(request.model.id)).midConversationSystem
...
// User, tool-result, and wrapped-system content all lower into the `user` wire
// role. Consecutive same-role turns are merged here because Bedrock-hosted
// Anthropic rejects them outright and the direct API only tolerates them by
// merging server-side. Appending preserves block order and every cache_control
// already attached to earlier blocks.
const appendUserBlocks = (messages: AnthropicMessage[], content: ReadonlyArray<AnthropicUserBlock>) => {
...
const lowerMessages = Effect.fn("AnthropicMessages.lowerMessages")(function* (
  ...
  for (const [index, message] of request.messages.entries()) {
    if (message.role === "system") {
      ...
      appendUserBlocks(messages, [
        { type: "text", text: part.text, cache_control: cacheControl(breakpoints, part.cache) },
      ])
      continue
    }
...
const lowerThinking = Effect.fn("AnthropicMessages.lowerThinking")(function* (
  request: LLMRequest,
  capabilities: AnthropicModel.Capabilities,
) {
  const thinking = anthropicOptions(request)?.thinking
  if (!ProviderShared.isRecord(thinking))
    return capabilities.adaptiveThinking === "default" || capabilities.adaptiveThinking === "required"
  ...
  if (thinking.type === "disabled") {
    if (capabilities.adaptiveThinking === "required")
      return yield* invalid(`Anthropic model ${request.model.id} requires adaptive thinking`)
    const effort = anthropicOptions(request)?.effort
    const maximum = capabilities.disableThinkingMaxEffort
    if (typeof effort === "string" && maximum !== undefined) {
      const current = capabilities.effort.indexOf(effort as AnthropicModel.Effort)
      const allowed = capabilities.effort.indexOf(maximum)
      if (current > allowed)
        return yield* invalid(`Anthropic model ${request.model.id} cannot disable thinking at effort ${effort}`)
    }
    return { type: "disabled" as const }
  }
  if (thinking.type !== "enabled") return undefined
  if (!capabilities.manualThinking && capabilities.adaptiveThinking !== "unsupported")
    return yield* invalid(`Anthropic model ${request.model.id} requires adaptive thinking`)
  const budget =
...
const outputConfig = Effect.fn("AnthropicMessages.outputConfig")(function* (
  request: LLMRequest,
  capabilities: AnthropicModel.Capabilities,
) {
  const effort = anthropicOptions(request)?.effort
  if (typeof effort !== "string") return undefined
  if (capabilities.effort.length > 0 && !capabilities.effort.includes(effort as AnthropicModel.Effort))
    return yield* invalid(`Anthropic model ${request.model.id} does not support effort ${effort}`)
  return { effort }
})
...
const fromRequest = Effect.fn("AnthropicMessages.fromRequest")(function* (request: LLMRequest) {
  ...
  const capabilities = AnthropicModel.capabilities(String(request.model.id))
  ...
  if (
    capabilities.sampling === "default-only" &&
    (generation?.temperature !== undefined || generation?.topP !== undefined || generation?.topK !== undefined)
  )
    yield* Effect.logWarning(
      `Anthropic model ${request.model.id} does not support non-default sampling; omitted temperature, topP, and topK`,
    )
  ...
  return {
    ...
    temperature: capabilities.sampling === "default-only" ? undefined : generation?.temperature,
    top_p: capabilities.sampling === "default-only" ? undefined : generation?.topP,
    top_k: capabilities.sampling === "default-only" ? undefined : generation?.topK,
    stop_sequences: generation?.stop,
    thinking: yield* lowerThinking(request, capabilities),
    output_config: yield* outputConfig(request, capabilities),
  }
})
...
const mergeUsage = (left: Usage | undefined, right: Usage | undefined) => {
  ...
  const normalized = ProviderShared.normalizeInputUsage({
    semantics: "exclusive-input",
    nonCached: right.nonCachedInputTokens ?? left.nonCachedInputTokens,
    cacheRead: right.cacheReadInputTokens ?? left.cacheReadInputTokens,
