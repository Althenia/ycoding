...
const anthropic = (schema: JsonSchema): JsonSchema => {
  const normalized = schema.type === undefined ? { ...schema, type: "object" } : schema
  if (!["anyOf", "oneOf", "allOf"].some((key) => Array.isArray(normalized[key]))) return normalized

  const { $defs, ...root } = normalized
  const definitions = isRecord($defs) ? $defs : {}
  const base = "__ycoding_root"
  let name = base
  let suffix = 0
