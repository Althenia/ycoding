...
describe("Bedrock Converse route", () => {
  ...
  // Bedrock rejects consecutive same-role turns outright, so lowering must emit
  // strictly alternating roles instead of relying on provider-side merging.
  ...
  it.effect("merges consecutive user messages into one alternating turn", () =>
    Effect.gen(function* () {
      ...
      expect(prepared.body.messages).toEqual([
        {
          role: "user",
          content: [{ text: "First." }, { cachePoint: { type: "default" } }, { text: "Second." }],
        },
      ])
    }),
  )
