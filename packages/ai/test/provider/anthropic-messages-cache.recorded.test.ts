...
import { CacheHint, LLM, type Usage } from "../../src"
...
describe("Anthropic Messages cache recorded", () => {
  recorded.effect.with("writes then reads cache_control on identical second call", { tags: ["cache"] }, () =>
    Effect.gen(function* () {
      ...
      // The recorded response also nests a per-TTL breakdown (`cache_creation.
      // ephemeral_5m_input_tokens` / `ephemeral_1h_input_tokens`) alongside the
      // flat `cache_creation_input_tokens` total. It must survive decoding into
      // `providerMetadata.anthropic` rather than being silently stripped as an
      // undeclared schema field.
      const anthropic = second.usage?.providerMetadata?.anthropic as
        | { cache_creation?: { ephemeral_5m_input_tokens?: number; ephemeral_1h_input_tokens?: number } }
        | undefined
      expect(anthropic?.cache_creation?.ephemeral_5m_input_tokens).toBeTypeOf("number")
      expect(anthropic?.cache_creation?.ephemeral_1h_input_tokens).toBeTypeOf("number")
    }),
  )
})
