...
describe("OpenAI Chat route", () => {
  ...
  it.effect("marks an explicit prompt_cache_breakpoint on cached system/user content for gpt-5.6+ only", () =>
    Effect.gen(function* () {
      ...
      expect(preparedLegacy.body.messages[0]).toEqual({ role: "system", content: "static prefix" })
      expect(preparedLegacy.body.messages[1]).toEqual({ role: "user", content: "hi" })
      ...
      expect(prepared.body.messages[0]).toEqual({
        role: "system",
        content: [{ type: "text", text: "static prefix", prompt_cache_breakpoint: { mode: "explicit" } }],
      })
  ...
  it.effect("caps explicit prompt_cache_breakpoint markers at 3 in implicit mode", () =>
    Effect.gen(function* () {
      ...
      const marked = prepared.body.messages.flatMap((message) =>
        "content" in message && Array.isArray(message.content)
          ? message.content.filter(
              (part): part is { type: "text"; text: string; prompt_cache_breakpoint: unknown } =>
    ...
    LLMClient.generate(
      ...
      Effect.provide(
        dynamicResponse((input) =>
          Effect.gen(function* () {
            const web = yield* HttpClientRequest.toWeb(input.request).pipe(Effect.orDie)
            expect(web.url).toBe("https://ycoding-test.openai.azure.com/openai/v1/chat/completions?api-version=v1")
  ...
  it.effect("surfaces a top-level cache_discount (OpenRouter) without failing decode", () =>
    Effect.gen(function* () {
      ...
      expect(response.usage?.providerMetadata?.openai).toMatchObject({ cache_discount: 0.0004 })
    }),
  )

  it.effect("omits cache_discount from provider metadata when the provider never sends it", () =>
    Effect.gen(function* () {
      const body = sseEvents(
        deltaChunk({ role: "assistant", content: "Hi" }),
        deltaChunk({}, "stop"),
      ...
      expect(response.usage?.providerMetadata?.openai).not.toHaveProperty("cache_discount")
    }),
  )
