...
const opus5 = AnthropicMessages.route
  .with({ endpoint: { baseURL: "https://api.anthropic.test/v1/" }, auth: Auth.header("x-api-key", "test") })
...
describe("Anthropic Messages route", () => {
  ...
  it.effect("applies approved Claude Opus 5 thinking, effort, and sampling rules", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
        LLM.request({
          model: opus5,
          prompt: "Hello.",
          generation: { temperature: 0.7, topP: 0.8, topK: 40 },
      ...
      const error = yield* LLMClient.prepare(
        LLM.request({
          model: opus5,
          prompt: "Hello.",
          providerOptions: { anthropic: { thinking: { type: "disabled" }, effort: "max" } },
        }),
      ).pipe(Effect.flip)
      expect(error.message).toContain("cannot disable thinking at effort max")
    }),
  )

  it.effect("requires adaptive thinking for Claude Fable 5", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
        LLM.request({ model: fable5, prompt: "Hello." }),
      )
      expect(prepared.body.thinking).toEqual({ type: "adaptive" })
      ...
      expect(error.message).toContain("requires adaptive thinking")
    }),
  )

  it.effect("moves Opus 5 custom tool unions behind an object-root reference", () =>
    Effect.gen(function* () {
      const anyOf = [
        {
          type: "object",
  ...
  it.effect("lowers chronological system updates natively for Claude Opus 5 and Fable 5", () =>
    Effect.gen(function* () {
      for (const current of [opus5, fable5]) {
        const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
          LLM.request({
            model: current,
            messages: [
        ...
        expect(prepared.body.messages).toEqual([
          { role: "user", content: [{ type: "text", text: "Before." }] },
          { role: "system", content: [{ type: "text", text: "Operator update." }] },
          { role: "assistant", content: [{ type: "text", text: "After." }] },
        ])
      }
    }),
  ...
  it.effect("merges a trailing user message into the preceding tool-result turn", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
        LLM.request({
          model,
          messages: [
            ...
            Message.user("Subagent TeamView"),
          ],
          cache: "none",
        }),
      )

      expect(prepared.body.messages.map((message) => message.role)).toEqual(["user", "assistant", "user"])
      expect(prepared.body.messages.at(-1)).toMatchObject({
        role: "user",
        content: [
          { type: "tool_result", tool_use_id: "call_1", cache_control: { type: "ephemeral" } },
          { type: "text", text: "Subagent TeamView" },
        ],
      })
    }),
  )

  it.effect("merges consecutive user messages into one alternating turn", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
        LLM.request({
          model,
          messages: [
            Message.user([{ type: "text", text: "First.", cache: new CacheHint({ type: "ephemeral" }) }]),
            Message.user("Second."),
          ],
          cache: "none",
        }),
