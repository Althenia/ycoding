...
// minimum cacheable-prefix threshold — see `src/cache-profile.ts` for the
// per-model figures; the highest is 4096. Built by repeating a fixed sentence —
// the cassette replays bit-for-bit, so the exact text matters only when
// re-recording with `RECORD=true`.
