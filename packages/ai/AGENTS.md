...
Keep this package independent of session concerns. Session auth, permissions, plugins, telemetry headers, cache namespace selection, and runtime orchestration belong in `packages/core/src/session/` and its runner adapters.
...
Catalog-selected native providers use package-like export paths from `@ycoding-ai/ai`. They are internal entrypoints in one npm package, not separately published provider packages. Every entrypoint implements `ProviderPackage.Definition` and exposes `model(modelID, settings)`, where settings are serializable provider configuration plus common `headers`, `body`, and `limits` overlays.
...
packages/ai/src/
  ...
  route/
    index.ts                @ycoding-ai/ai/route advanced barrel
...
Native chronological system messages are route/model-specific. Anthropic Messages lowers them natively for Claude Opus 4.8 (`claude-opus-4-8`), and the ChatGPT/Codex Responses route preserves trusted text-only updates as chronological native `system` input. Other routes and models intentionally lower the update in place into ordinary user-compatible text using this stable escaped representation:
