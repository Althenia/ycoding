...
> the current private `@ycoding-ai/ai` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@ycoding-ai/ai`
...
- Secondary audience: YCoding and other durable agent runtimes
...
import { OpenAI } from "@ycoding-ai/ai/providers/openai"
...
import { LLM, Tool } from "@ycoding-ai/ai"
...
YCoding and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@ycoding-ai/ai/promise"
import { OpenAI } from "@ycoding-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@ycoding-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@ycoding-ai/ai/provider"
...
@ycoding-ai/ai/promise
  Promise/AsyncIterable LLM facade

@ycoding-ai/ai/schema
  serializable domain schemas

@ycoding-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@ycoding-ai/ai/providers/openai
@ycoding-ai/ai/providers/anthropic
@ycoding-ai/ai/providers/google
...
```
...
YCoding should migrate to `generateTurn` / `streamTurn`, preserving its durable
