...
test("replaces a failed registered service", async () => {
  const registration = await setup("failed-owner")
  const failed = await Bun.file(registration).json()
  ...
  try {
    expect(endpoint.url).toBe(info.url)
    expect(info.pid).not.toBe(failed.pid)
    await waitForExit(failed.pid)
  } finally {
    process.kill(info.pid, "SIGTERM")
    await waitForExit(info.pid)
  }
...
async function temp() {
  const directory = await mkdtemp(join(tmpdir(), "ycoding-promise-service-"))
