...
describe("HttpApiCodegen.generate", () => {
  ...
  test("executes nested Promise operation IDs", async () => {
    ...
    const directory = await mkdtemp(join(tmpdir(), "ycoding-httpapi-codegen-"))
    ...
    try {
      ...
      const client = generated.YCoding.make({
  ...
  test("serializes an opaque union payload as the direct JSON body", async () => {
    ...
    try {
      ...
      await client.session.configure({ payload: { type: "local", command: ["ycoding"] } })

      expect(await request?.json()).toEqual({ type: "local", command: ["ycoding"] })
    } finally {
      await rm(directory, { recursive: true, force: true })
    }
  })
