...
if (!repo) throw new Error("GH_REPO must be set to owner/repository")
...
const order = ["Runtime", "TUI", "API", "Plugins", "UI"] as const
const sections = {
  runtime: "Runtime",
  tui: "TUI",
  api: "API",
...
async function commits(from: string, to: string) {
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/tui/") || file.startsWith("packages/cli/")) areas.add("tui")
