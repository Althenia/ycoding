...
The built-in `conversation_compact` tool is the current advisory entry point for local selective compaction. It accepts no input fields. At configured `consider` or `advised` pressure it durably admits a background job and returns `{ "jobID": "cmp_...", "status": "scheduled" }`; below advisory pressure, or when `compaction.advisory` is `false`, it returns `{ "status": "disabled" }`. The worker validates a ContextManifest and activates exclusions through a new immutable context revision without deleting canonical messages or transcript events.
