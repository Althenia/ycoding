...
`GSD` (Get shit done) is a one-shot delivery agent: it finds the root cause, implements the highest-impact complete solution directly when fastest, and delegates independent tasks to background subagents in parallel when that shortens delivery. It follows repository standards, assigns one writer per mutable resource, and keeps dependencies and approvals as gates. It uses TDD for meaningful behavioral mismatches, adds only high-impact test cases, runs focused tests rather than a full suite, and verifies completion against affected checks and the final diff. This role is expressed through its instructions, not a separate tool-permission sandbox.
