...
The Location-scoped `computer` service separates platform-neutral capability reporting, Session ownership, revision fencing, cancellation, permissions, and guardrails from native providers. macOS is the only platform provider. It supports exact iTerm session inspection and text submission, exact Finder path inspection and move, and one explicitly identified running application's window through bounded Accessibility inspection, window capture, and AX control actions. Unsupported platforms expose no capabilities and fail before helper invocation.

Inspect claims one explicit target for the observing Session and returns its revision. A mutation requires the same Session and revision. Session release, cancellation, a stale revision, cross-Session access, or an uncertain native result prevents mutation until a fresh inspect. iTerm text passes through shell guardrails, while Finder moves use canonical Location paths, external-directory permissions when applicable, and file-mutation guardrails. Every desktop read and write requires the `computer` permission before dispatch; the standard guardrail profile adds no desktop review, and custom guardrail rules for the `computer` action still apply.

The macOS helper never activates or launches target applications, targets frontmost UI, uses global input or clipboard state, reveals Finder items, or captures another app/window. Every operation runs in the signed sibling app bundle, which holds the macOS Accessibility, Screen Recording, and Automation grants; a missing grant makes macOS list the app and ask the user, and the operation fails closed until the user allows it and retries. Bun source development resolves only explicitly built helpers in Core's repository-ignored cache and never compiles them during runtime startup. See [Native computer use](./computer-use.md) for the exact capability, build, install, and validation boundaries.
