...
`ycoding remote connect` runs one foreground outbound agent connection to the configured relay until the operator stops it or the process exits. The agent dials out over WSS and never listens. Every local call is addressed from the locally opted-in sharing list, never from remote input, and execution authority stays in the local `ycoding` process; the relay and the browser own no repository, shell, tool, model, or Session execution.

The sharing list is the `remote.json` allowlist. The agent advertises those Session IDs on every successful open, including each reconnect. It re-reads the allowlist before each request and each forwarded event, re-verifies every Session at its recorded Location on every execution, and drops a Session that moved or no longer exists from the advertisement until the local user shares it again; a moved Session is never followed implicitly. `session.list` answers from the allowlist intersection, `session.active` and guardrail review lists are filtered to the advertised set, and `session.get` returns the verified Session info, so a remote client cannot enumerate local Sessions.

Inbound frames pass the shared envelope parser before any local call, and every input field is validated before any local side effect. Mutations are attempted exactly once: a prompt carries its durable message ID unchanged so an exact local retry reconciles, approval replies carry no key, and an indeterminate outcome is reported as `outcome_unknown` instead of being replayed. A permission or question reply is refused once the request is no longer pending. Guardrail review lists and replies are restricted to reviews owned by an advertised Session, so sharing a root Session never exposes a child Session's reviews.

One local event-stream subscription serves the connection, refcounted by Session interest so one client's unsubscribe cannot tear down another client's stream. Event arrivals are serialized and forwarded verbatim, once, in arrival order, only after the current allowlist read succeeds and confirms that the Session is both advertised and subscribed. A pending or failed allowlist read forwards nothing, and queued events remain bound to the connection that received them so replacement cannot send stale work. The pending authorization queue is bounded; overflow replaces the connection so the client reconciles from snapshot and history instead of accepting a partial event sequence. Heartbeat pings keep the socket alive. A closed or refused socket reconnects with bounded exponential backoff, starting at one second and doubling to thirty seconds, and re-advertises the allowlist; a credential rejected with `4401` or `4403` rotates once, and a device the relay refuses again stops the agent as terminal instead of retrying indefinitely.
...
The relay exposes no filesystem read and no caller-selected path. Captured shell output is reachable only through the paged `session.shell.output` operation, whose input is `{ shellID, cursor?, limit? }` and whose result is `{ data: Shell.Output }` with `{ output, cursor, size, truncated }`. `limit` is a positive integer defaulting to and capped at 65 536 bytes, `cursor` is a non-negative integer, and one request returns one page; a reader pages explicitly by the returned cursor, and incompleteness is `cursor < size` because the local page read never reports `truncated`. A page can return no new output at the cursor it returns while a running command's capture ends inside a character the command has not finished writing, and it never advances into that character, so a reader continues when the cursor moves and re-reads otherwise. The operation is addressed by an advertised Session ID and served only when the shell's own recorded `metadata.sessionID` matches that Session, re-verified at the Session's bound Location on every request. Direct Session shells and Session tool shells record their owning Session; a shell owned by another Session, a shell with no recorded owner, and an unknown shell are all refused before any output byte is read.
