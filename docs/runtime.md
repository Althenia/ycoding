...
`session.context.observed.1` is an append-only durable Session event with `{ sessionID, source, text }`. `source` is one of `session-state`, `team-view`, or `step-limit`. Its projection is chronological: `session-state` and `step-limit` become trusted System messages, and `team-view` becomes a Synthetic message with user authority and the description `TeamView update`. Session-state and TeamView notices show compact summaries by default. Their details expand into a scrollable Markdown table view capped at one third of terminal height (minimum three lines); the `session.context_details` setting in `cli.json` enables expanded details by default. Click a notice to toggle it, or use Enter/Space when focused. Arrow keys, Page Up/Down, and Home/End scroll expanded details. This is presentation only: stored event/message text and model-facing history remain unchanged.
