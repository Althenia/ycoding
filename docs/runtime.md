...
The built-in `memory` tool accesses explicit linked Markdown knowledge. Repository memories are keyed by the canonical Git common directory, so the main checkout, its subdirectories, symlink aliases, and every linked worktree share one repository collection; shared knowledge lives under `knowledge/` and remains available outside Git. The tool has no automatic transcript extraction or concept injection into prompts. Built-in `ycoding/workspace-memory` Session guidance directs agents to search memory before substantive work and to save newly verified durable facts after validated completion through the same tool. Permissions protect reads and writes; writes and offline HTML graph export also use the existing file-mutation guardrail. Concepts remain authoritative files, independent of durable Session events and managed project artifacts. See [Workspace knowledge memory](./memory.md) for the configuration, format, retrieval, and conflict contracts.
...
One toggle command controls the selection: it is visible in the landing and Session command palettes and also runs as the `/daybreak` slash command with the optional arguments `blue`, `red`, and `off`. Without an argument it cycles off → blue → red → off, offering only programs the active model advertises and skipping programs it does not advertise. The model picker lists ordinary models only; Daybreak state comes from the toggle, not from picking a special model. Catalog discovery and provider request fields are specified in [configuration](./configuration.md).

On the landing screen, the selection belongs to the current draft screen and appears beside the model in the header without creating a Session. Submitting a prompt captures that selection with the draft, creates the Session, and saves Daybreak before admitting or waking the prompt. An explicit landing `/goal <objective>` saves the selection before activating the goal. Failed Daybreak persistence starts no model work; the draft remains editable, and retrying an unchanged prompt reuses the confirmed Session. Returning to a fresh landing screen starts with Daybreak off.

The Session header places `Daybreak Blue` or `Daybreak Red` immediately after the current model name when the selected program is advertised for that OpenAI model. A saved selection displays `(inactive)` on an unsupported model or another provider, including GPT through OpenRouter, Anthropic, and DeepSeek. Model switches and catalog refreshes update the indicator from the current durable Session and model catalog; a pending model choice does not change the current indicator. Returning to a supported model restores the indicator without changing the saved selection. `/daybreak off` clears the selection and removes the indicator. The indicator describes the selected request mode, not confirmation of provider authorization or expanded local tool permissions.
...
The browser's Session list retains the reported `projectID` and `location.directory` from each page. These fields describe the backend's placement; they do not authorize a browser-selected Location. Missing or malformed metadata remains absent rather than being inferred from a Session title. Superseded inventory reads stop before requesting another page.

The browser paces outbound frames within the relay's shared limit of 30 frames per 10 seconds, so large inventories can finish across several windows without a policy disconnect. At most 32 browser requests may be waiting or in flight; their outcome timeout starts when transmitted. Delayed requests belong to the current live socket: closing it cancels unsent requests as unavailable and reports transmitted, unsettled requests as unknown. Neither is replayed on reconnect, and requests made while offline are refused.

`workspace.list` lists existing directories from recorded ProjectDirectory entries, Session Locations, and non-global Project worktrees. The global non-Git Project's worktree is not a workspace choice. Each choice carries an opaque ID, project ID, directory, and optional project name; enumeration does not initialize every Location. `GET /api/location` records an existing opened directory in the current project tables without creating a Session, overwriting project metadata, or replacing a directory's strategy. Missing directories are not recorded. Historical directories with no retained project or Session record must be opened locally once to enter this inventory.

`session.create` accepts only `{ id, workspace }`, where `workspace` is an ID from that backend's inventory. The agent rebuilds the inventory, checks the directory and its current Location project identity, and creates an idle root Session. It verifies returned placement before advertising success; a retry adopts only the same root ID at the same project and Location. Filesystem changes are not atomic with creation. Uncertain results retain the requested ID; an explicit browser retry checks for that Session before resending creation. The browser never sends a filesystem path or creates a missing repository. Agent and model selection follow the new Session's local defaults when execution starts.

Selecting an existing Session or completing creation opens Conversation without admitting a prompt. Browser drafts remain in memory per Session while navigating remote routes or reconnecting to the same machine. A different machine, explicit disconnect, sign-out, or workspace disposal clears those drafts. A creation that finishes after its dialog closes updates the Session list without changing the user's route or selection.
...
The browser remote transcript renders at most 4,000 characters of text per tool row across its content blocks. It replaces generated capture-path truncation markers with path-free text and labels source truncation separately from the browser display cap. This presentation limit does not change durable Session history, the relay's response chunks, or paged shell output.
...
Native HTTP/SSE framing ignores `retry:` reconnect hints without closing or replaying the active response. It preserves subsequent content, completion, usage, and read errors across byte boundaries and mixed LF/CRLF line endings. Empty SSE data and `[DONE]` markers are not provider completion. A normalized stream that ends without a terminal event fails as a transport read error and follows the bounded recovery policy; it never synthesizes success or executes an unfinished tool call.

Responses routes reconcile assistant text from deltas, finalized text/content parts, completed message items, and the terminal response output. Completion snapshots append only missing text; repeated snapshots do not duplicate text or streamed tool calls. Content-part identity and assistant phase are preserved. A snapshot that contradicts already streamed text fails as invalid provider output rather than rewriting delivered content. A valid answer supplied only at completion is a normal response, not terminal silence, and creates no extra recovery request.
...
Settled-silence recovery uses one additional logical `step` request recorded as a full request, not as a physical retry or continuation fallback. Its tools-disabled text-only mode remains distinct from provider recovery, which follows normal Step allowance and tool rules and is bounded by next-Step recovery mode rather than one allowance for the whole drain. The settled-silence tool-prefix change may produce the existing cache-invalidation vocabulary.
...
Model-based titles and goals resolve the hidden agent's explicit model first, then the matching `efficiency.helper_models.title` or `.goal` selection. Local compaction resolves `efficiency.helper_models.compaction.main` for a main chat or `.subagent` for a child Session. An explicit configured value wins over an agent-pinned model; a missing value or `session` retains agent-model then owner-Session-model precedence. Local jobs reuse a deterministic taskless child Session with the selected model and the hidden built-in `compaction` agent. That agent remains `mode: primary`; the child is internal provider identity, not a managed subagent task. Its provider requests and usage ledger remain isolated from the owner Session. Codex remote compaction uses the owner Session's model and request identity instead of the helper; failed remote attempts fall back to the configured local helper.
...
Activation stores a content-addressed manifest, appends one immutable context revision, replaces only the active exclusion projection, rebases opaque provider state, and invalidates stored Responses continuation. A non-empty valid local summary counts as retained model context and may cover the manifest's full boundary when no protected recent tail overlaps it. Local model-history reads prepend one stable, non-persisted synthetic `<conversation-checkpoint>` derived from the active manifest digest and activation time, then retain only later selected entries. A Codex remote manifest instead retains bounded real user messages, a private content-addressed encrypted compaction item, and post-boundary history; only the matching provider, model, and variant can replay that item. A different model selects canonical history without ciphertext. Covered source messages are never modified. The compaction lifecycle is projected as one durable message keyed by `jobID`, created at admission and updated in place through completion or failure, so Protocol and TUI transcript reads can rehydrate its boundary after process restart.

The Step runner estimates the safe input budget `context - compaction.context_safety_margin_tokens` before an ordinary provider request. For stateless Responses with a usable provider compaction boundary, it estimates the effective pruned input rather than the complete local transcript and does not reuse pre-boundary provider totals to trigger another local checkpoint. It trips the mandatory gate when that local chars/4 estimate, the applicable last provider-reported input-side total (uncached input + cache read + cache write from the latest assistant step), or the applicable last provider-reported full total (input + output + reasoning + cache read + cache write from the latest assistant step) reaches that budget, then starts or joins a mandatory compaction job and reloads after settlement. Missing model limits, unavailable boundaries, unchanged compaction failures, and insufficient reduction never block the provider request; the gate performs at most two owned admissions before proceeding with its latest rebuilt request.
...
An optional unpacked Manifest V3 extension exposes eligible existing and future Chrome tabs automatically after pairing, alongside newly created agent-owned background tabs. Pairing begins from an active bootstrap Session using a two-minute, one-time secret in the first WebSocket frame and an exact matching `chrome-extension://` Origin; the WebSocket URL itself contains no credential. Core shares one process-global bridge across active Sessions and Locations. Caller-specific model permissions and guardrail deny rules still apply; browser tool calls are never guardrail-reviewed. The Session command palette exposes **Connect Chrome** to create the one-time code; its popup uses the backend-wide `/api/browser/connect` WebSocket without a Session ID. A durable extension credential survives the initiating Session being archived, moved, or deleted. Any active Session can forget it.

The browser tool exposes status, tab listing, bounded accessibility observations, generation-fenced semantic actions, pause/resume, and stop. It does not expose pairing controls, arbitrary JavaScript, arbitrary CDP, cookies, storage, host input, clipboard, explicit downloads/uploads, history, or unrestricted URL data. Omitted mode controls eligible paired Chrome tabs, including the active tab; explicit `mode: "profile"` selects the same tabs, `mode: "owned"` may open and close only newly agent-created tabs belonging to the calling Session, and `mode: "isolated"` selects the already-started temporary browser. Chrome's native YCoding debugger banner signals attachment, and its Cancel revokes that tab until pairing is forgotten and renewed. Profile list/observe/capture/scroll require site read permissions without a standard profile-access review; profile mutations require site permissions. Group changes additionally require control permission and are limited to inactive, granted tabs. Model-driven owned open requires target-site navigation and read permissions. Foreground observe and semantic actions remain available; active owned tabs cannot be closed or automatically cleaned up. Explicit requested pauses and uncertain/disconnected states still block control. Navigate/click/type require source-site and known destination-site approval; the approval warns that actions may indirectly trigger downloads. Unknown page-script destinations cause revocation before their metadata reaches the model, not prevention of the navigation. An explicit pre-dispatch error settles as rejected; mutation failures after dispatch without a known terminal result remain uncertain. Both retain the original call ID and prevent automatic replay. See [Chrome browser bridge](./browser-extension.md) and the [V2 contract](../specs/v2/browser.md) for exact bounds.

### Session-owned isolated browser

The Session command palette exposes **Isolated browser** controls for explicit temporary headless browsing. The user starts a fresh browser with a credential-free HTTP(S) URL; opening or refreshing the dialog never starts it. There is no personal-profile attachment, persistent login, visible-browser handoff, or automatic restart. Isolated mode requires macOS arm64 with installed Google Chrome major 152 or newer. Launch fails closed if its required safety controls cannot be configured; older or unavailable installations report it unavailable.

The `browser` tool uses explicit `mode: "isolated"` to select this already-started browser. Omitted mode selects paired Chrome's eligible profile tabs and never falls back. Isolated status and control results containing page metadata require `browser_read` before model exposure, independently of `browser_control`; semantic mutations retain their permission checks and `browser_mutation` guardrail deny evaluation without review. Normal authenticated Session-location routes and regenerated clients expose the separate `isolatedBrowser` group.
...
The Location-scoped `computer` service separates platform-neutral capability reporting, Session ownership, revision fencing, cancellation, permissions, and guardrails from native providers. macOS is the only platform provider. It supports exact iTerm session inspection and text submission, exact Finder path inspection and move, running application and window discovery, background application launch and graceful quit, and one explicitly identified running application's window through bounded Accessibility inspection, window capture, AX control actions, and pointer and keyboard events targeted at that application's process. Off-Space Electron windows use a debug bridge owned by the exact process for capture and content input. An explicitly staged window moves onto a private agent display, owned by a helper process per Location, until it is unstaged, its Session is released, or the Location shuts down. Safari and Chrome tabs are listed and controlled through fixed Apple Event templates. Unsupported platforms expose no capabilities and fail before helper invocation.

Inspect claims one explicit target for the observing Session and returns its revision. A mutation requires the same Session and revision. Session release, cancellation, a stale revision, cross-Session access, or an uncertain native result prevents mutation until a fresh inspect. iTerm text passes through shell guardrail deny evaluation, while Finder moves use canonical Location paths, external-directory permissions when applicable, and file-mutation guardrail deny evaluation. Every desktop read and write requires the `computer` permission before dispatch. Computer tool calls are never guardrail-reviewed; standard and custom deny rules still block them.

The macOS helper launches applications only without activating them, and never activates or raises target applications, changes the frontmost application, moves the hardware cursor, posts global input, uses clipboard state, reveals Finder items, or captures a window other than the explicit target. The Electron bridge opens a per-call localhost inspector only in applications whose inspect fuse is on, and an explicit `remote_debugging` launch gracefully quits and relaunches an application with a localhost renderer debug port that stays open until it quits. Every operation runs in the signed sibling app bundle, which holds the macOS Accessibility, Screen Recording, and Automation grants; a missing grant makes macOS list the app and ask the user, and the operation fails closed until the user allows it and retries. Bun source development resolves only explicitly built helpers in Core's repository-ignored cache and never compiles them during runtime startup. See [Native computer use](./computer-use.md) for the exact capability, build, install, and validation boundaries.
...
Anthropic-compatible requests use one fixed TTL per request source, because changing a breakpoint's TTL within a Session misses every cache entry written earlier. Under the default `adaptive` setting, root Session steps on an extended-TTL-capable model use one hour from the first request. Child Session steps and title, goal, and compaction requests use five minutes. Unknown or unsupported model profiles, including unknown future models on direct and compatible Anthropic routes, use five minutes. OpenRouter cache policy is profile-gated: the native route sends top-level automatic `cache_control` only for Anthropic models, with the one-hour bucket only on profiled models, while the AI SDK route lowers bounded inline markers. Inline-marker routes anchor the newest cacheable message and the last cacheable message before the latest assistant message, so one anchor lands exactly on the previous request's final breakpoint even when a large parallel tool fan-out places the newest message beyond Anthropic's 20-block lookback.

Direct OpenAI Responses requests authenticated with a public API key use GPT-5.6-and-later prompt-cache options and exact `input_text` breakpoints. YCoding keeps one combined system-text breakpoint plus the newest eligible non-volatile user and local tool-result text boundaries within OpenAI's latest-50 read-candidate window. Assistant replay always uses `output_text` and never receives a breakpoint. A marked local tool result uses an `input_text` block inside `function_call_output.output`; structured media remains in its existing provider-native blocks. Tool definitions, tool calls, provider-executed tool results, and plugin-owned volatile messages receive no generated marker. `openai_mode: "auto"` normally sends request-wide `{ mode: "implicit", ttl: "30m" }`, reserves one read candidate for OpenAI's managed implicit breakpoint, and emits at most 49 explicit candidates. When a plugin-owned volatile message trails the request, the compiled request switches to `{ mode: "explicit", ttl: "30m" }`, retains the volatile suffix without a generated marker, and emits at most 50 explicit candidates before it. `openai_mode: "explicit"` disables that managed breakpoint and emits at most 50 explicit candidates. OpenAI can write the latest three explicit candidates plus the managed breakpoint in implicit mode, or the latest four explicit candidates in explicit mode. `30m` is a minimum reuse lifetime, not a hard expiry; OpenAI may retain state longer up to its separate 24-hour maximum. Older public OpenAI models remain implicit and use opt-in `24h` retention only on supported families. The ChatGPT/Codex subscription backend has separate `openai-codex-responses` HTTP/SSE and `openai-codex-websocket-responses` WebSocket identities and is key-only for every model: YCoding emits no `prompt_cache_breakpoint`, `prompt_cache_options`, or `prompt_cache_retention`. Codex sends `session-id`, `thread-id`, and `x-client-request-id` equal to the Session-scoped prompt-cache key and the explicit WebSocket route uses that key for process-local affinity, so each Session keeps its own backend affinity, matching the official Codex client's per-conversation `session-id`. Codex reasoning models request `include: ["reasoning.encrypted_content"]` with or without a selected effort variant, so stateless replay keeps their reasoning items. Request-shape tests verify serialization only; Codex retention and cache reuse remain provider-controlled and no hit rate is guaranteed. Compatible gateways and unsupported families also omit GPT-5.6-only fields. Model-based title, goal, and compaction calls use the same policy and observation runtime as normal Session steps.
...
The user owns the objective. `/goal <text>` explicitly creates or replaces it, even while a goal is active. From the landing composer, it creates a Session and sets the goal on that Session before navigating to its transcript. Tracked pasted text is expanded before calculation. Ordinary prompts, exact retries, synthetic continuations, and agent goal-tool calls do not replace the objective or reset its no-progress budget. The agent retains inspection, reporting, completion, and stop actions, but cannot supply replacement text or activate a stopped goal.
...
- `computer` and `browser` tool calls are never reviewed: their ordinary and hard review decisions are admitted without a request, while deny decisions still block them;
...
The built-in TUI attention lifecycle keeps routine child completion and settlement while a goal remains active silent. Pending human-input requests still alert once per unresolved request. A normal Session produces a local completion alert only when its own `task_complete` tool call succeeds and the root settles with no active child task or root-owned running shell; ordinary idle and successful remote `ntfy` delivery are not local completion signals. A goal completed from active state also produces a completion alert after settlement and the same unfinished-work check. Top-level completion and exhausted-goal alerts are coalesced across successor executions; a retained terminal goal from earlier work does not determine the alert for a later ordinary execution. Global MCP forms are local-only: the TUI may use desktop and sound attention, while Core's remote ntfy lifecycle does not post them or grant additional authority.

Core runs the remote ntfy lifecycle through one process-global observer. For each Session event it resolves the owning Location, then reads that Location's ntfy configuration, effective permission, and HTTP delivery service. The observer waits 500 ms before eligible delivery and requires an already-effective `allow`; it does not prompt for `ask`. Automatic successful-terminal delivery requires an active goal reaching a terminal state; completed goals with active child tasks or root-owned running shells and ordinary successful idle are silent. A completed explicit ntfy tool call after an execution starts suppresses that execution's automatic successful-terminal post only. It does not suppress other attention categories or define general notification deduplication.
...
`GSD` (Get shit done) is an all-round agent: it answers read-only requests without edits and delivers requested changes by the shortest safe path, finding the root cause where relevant. It implements directly when fastest and delegates independent tasks to background subagents when their parallel progress outweighs delegation overhead on the critical path. It dispatches ready independent work early with bounded outcomes, ownership, and acceptance checks; tightly coupled edits and integration stay with the primary agent. It continues independent work while children run, then verifies their evidence. It follows repository standards, assigns one writer per mutable resource, and keeps dependencies and approvals as gates. It uses TDD for changed behavior, runs focused tests and required affected checks, and verifies completion against the final diff. This role is expressed through its instructions, not a separate tool-permission sandbox.

Each selectable primary agent speaks in its own role-appropriate style while grounding claims in evidence: GSD reports outcomes briskly without a status template; architech leads with a decision and material system trade-offs; god speaks with quiet authority and decisive evidence; yangi uses spare sentences and stops when the point is proved. Changed-work reports include paths and exact checks; material blockers and risks are stated when present.
...
OpenAI-compatible Chat usage normalizes nested `prompt_tokens_details.cached_tokens`
as cache reads and falls back to DeepSeek's top-level `prompt_cache_hit_tokens`
when the nested count is absent. Meta Muse Spark runs on the native Responses
route with stateless `store: false` replay; its reasoning models request
`include: ["reasoning.encrypted_content"]` so encrypted reasoning is replayed, and
enabling `openai_extended_retention` sends Meta's `24h` `prompt_cache_retention`
hint.

For text-only models, the image-analysis fallback reuses each successful
description per image content digest, vision model, and effective prompt for the
life of the runtime process, so later steps replay byte-identical user messages
and make no further vision request for that image. Failed analyses are retried
on a later step, and a runtime restart analyzes each image once more.

### OpenAI

For GPT-5.6 and later, direct public OpenAI Responses lowering supports `prompt_cache_options` and explicit `prompt_cache_breakpoint` fields on system, user, and local tool-result input blocks; assistant replay always remains unmarked `output_text`. Auto mode combines explicit stable-prefix markers with OpenAI's implicit latest-message marker; explicit mode disables the managed marker. Pre-5.6 public models use the compatible retention field. ChatGPT Codex requests are key-only for every model and emit no `prompt_cache_breakpoint`, `prompt_cache_options`, or `prompt_cache_retention`. Their cache, thread, and client-request identities use the Session-scoped prompt-cache key; the explicitly selected WebSocket route also uses it for process-local socket affinity. Request-shape tests do not establish live Codex reuse, which remains provider-controlled without a hit-rate guarantee. Direct OpenAI is Responses-only; OpenAI-compatible Chat remains available only for configured third-party deployments. Model family and route capability are gated independently because sending the wrong cache field can be rejected by the provider.
...
GitHub Copilot Responses through the AI SDK route is explicitly stateless (`store: false`). Every follow-up sends full selected history instead of `previous_response_id` continuation or provider-stored item references. Retained encrypted reasoning items and local tool outputs are replayed directly, and the route still receives the stable Session-scoped `prompt_cache_key`; Copilot-specific options override generic Responses options when both are present. Models that advertise reasoning effort request `include: ["reasoning.encrypted_content"]` even without a selected effort variant, and reported `cache_write_tokens` are normalized as cache writes. Copilot reasoning-summary fragments close before the next fragment starts, while the final closure retains encrypted reasoning metadata for replay. Copilot Responses models outside the `gpt-5`, `gpt-5.1`, and `gpt-5.2` families send `context_management: [{ type: "compaction", compact_threshold }]` at 90% of the advertised prompt limit (50,000 tokens when unreported). A returned compaction item becomes an empty reasoning part whose encrypted item is stored as private provider state; later stateless requests replay the latest item and omit everything before it while still sending current instructions and tools. Claude models that Copilot serves only through Chat Completions receive message-level `copilot_cache_control: { type: "ephemeral" }` on the system prompt and the two rolling message anchors, at most four markers.
...
Anthropic cache-control placement is normalized across direct and compatible provider routes. Anthropic models on native OpenRouter use top-level automatic `cache_control`, with the one-hour TTL only on profiled models, plus stable session affinity; other native OpenRouter model families receive no top-level `cache_control`, and the AI SDK OpenRouter route uses bounded inline markers. Native OpenRouter Responses replay keeps reasoning items, assistant `phase`, and tool metadata in the OpenAI Responses metadata namespace; OpenAI GPT-5.6-and-later models there also request `reasoning.encrypted_content` and `reasoning.context: "all_turns"` and send request-wide `prompt_cache_options` only in explicit mode. Plugin-owned volatile suffixes and context-pressure advisories do not receive cache breakpoints. Claude Code OAuth requests derive `x-claude-code-session-id` from the stable incoming `X-Session-Id`; an explicit interceptor override wins, and process-random identity is only the fallback when neither value exists. The OAuth billing system prefix samples the first durable canonical user message for that Session, so local compaction and runtime recreation do not replace its fingerprint with checkpoint text. A missing or unreadable durable Session falls back to the current visible first-user sample. Public Anthropic API-key requests do not use this Claude Code translation. Direct Anthropic Messages preserves thinking blocks whose visible text is empty but that carry a signature, and `redacted_thinking` blocks, as reasoning parts with provider metadata and replays them unchanged; neither becomes visible text or receives a cache breakpoint.
...
Left/Right or Tab/Shift+Tab switches views. Up/Down selects report rows or scrolls Overview and quota windows; Home/End and PageUp/PageDown navigate the current body. Enter opens selected-row details; Escape closes details before returning to chat. Bracket keys and paging controls load bounded 100-row report pages. Switching report views or changing a range or sort resets paging. Daily, Hourly, and Monthly open in ascending date order; Models, Sessions, Projects, and Agents open by descending total tokens. `d`, `t`, and `c` sort the full filtered dataset by identity/date, total tokens, and cost; repeating the current sort reverses its order, and each view keeps its own sort. Date-range controls offer Last 7d, Last 30d, All, and custom `YYYY-MM-DD..YYYY-MM-DD` bounds. Presets include the current UTC day; custom From is inclusive and To exclusive. Totals cover the full filtered scope rather than the current page.

Wide tables allocate remaining width to row identities and share header/row geometry with right-aligned columns STEPS, INPUT, OUTPUT, REASON, CACHE READ, CACHE WRITE, TOTAL, and COST, each in a distinct metric color. STEPS counts logical model calls. Values below 100,000 render exactly; larger token values render compactly (`271.4K`, `103.6M`, `1.2B`) so each row stays on one line, and details show exact values. A footer legend explains the columns. Costs render as `$0.00` without provenance suffixes; unpriced costs render `-`. Narrow layouts retain identity and step/token/cost details on separate rows. Cache reads that no contributing request reported render `-`; partially reported cache reads and the totals that include them render with a `≥` lower-bound prefix. Durable visible output and reasoning are disjoint categories; total tokens include each once.

Stats renders a colored activity graph over Sunday-aligned UTC calendar weeks: up to 52 weeks of 3-column cells when the terminal is wide enough, otherwise 2-column cells and as many recent weeks as fit. A month header marks each month's first week, future cells stay blank, and five theme-derived shades (empty through peak) encode each day's tokens relative to the busiest retained day. Below the graph, Stats lists the favorite model by tokens, Session count, current and longest daily streaks, active days over the 52-week window, total tokens, total cost, and busiest day, and states that usage before the first retained day is unreported. Activity counters describe recorded requests, not inferred user activity. Refresh preserves same-query results and row identity with explicit stale/loading/error labels; late responses cannot replace a newer query or an unmounted screen.
...
The Usage screen separates backend-wide retained statistics from provider quota windows; see [report controls](#telemetry). `GET /api/provider/usage` lists each provider available in the current Location's catalog, including connected providers unused by any Session, while honoring disabled providers and provider policy; a provider with a quota adapter and several stored credential profiles reports one snapshot per profile. Independent provider refreshes run with concurrency bounded to four; a failed provider does not discard successful snapshots. The quota view preserves Spark and other named windows and shows source, stability, freshness, reset times, and ten-cell progress bars. Unknown values remain unreported; reported zeros remain visible. Failed refreshes retain prior snapshots as stale and surface the partial failure. The API reports providers without a supported quota path or usable credential with an explicit `unsupported` status; the quota view hides them. Each profile of a multi-profile provider renders as its own section named by its profile. Unreported steps, reasoning, cache reads, token totals, and report costs render as `-`.

Provider failure rows render the provider's structured error message through the safe display sanitizer. Ordinary safe text remains visible. When details must be omitted, the normalized error category supplies an explanation such as authentication failure, interrupted connection, rate limit, or internal provider error; unknown categories use a generic failure message. Request identifiers and sensitive payloads remain hidden. Interrupted steps retain their existing presentation. The Session header renders only the generic `provider error` status and never repeats the detailed provider message.
...
The team composer is a separate picker from the sidebar `SUBAGENTS` section. Its tabs are ordered **Subagents**, **Shell**, **Side chats**. The Subagents tab shows active tasks under **ACTIVE** and completed, cancelled, failed, or lost tasks under **INACTIVE**. It filters only the resident page; it does not fetch other pages automatically. Paging controls and matching keyboard hints remain available when the current page has no tasks. Elapsed duration is measured from task creation to its update time after terminal settlement and then remains frozen. Selecting a task attaches to its child Session. A parent message steered or queued to a terminal child can reactivate it, preserving its Session context.
