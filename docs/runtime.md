...
The macOS helper launches applications only without activating them, and never activates or raises target applications, changes the frontmost application, moves the hardware cursor, posts global input, uses clipboard state, reveals Finder items, or captures a window other than the explicit target. Every operation runs in the signed sibling app bundle, which holds the macOS Accessibility, Screen Recording, and Automation grants; a missing grant makes macOS list the app and ask the user, and the operation fails closed until the user allows it and retries. Bun source development resolves only explicitly built helpers in Core's repository-ignored cache and never compiles them during runtime startup. See [Native computer use](./computer-use.md) for the exact capability, build, install, and validation boundaries.
