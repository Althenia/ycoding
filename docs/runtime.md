...
Settled-silence recovery uses one additional logical `step` request recorded as a full request, not as a physical retry or continuation fallback. Its tool-prefix change may produce the existing cache-invalidation vocabulary.
...
- Parent TeamView context is injected as a volatile message after durable history only while at least one direct child is non-terminal and the resolved route is not the key-only ChatGPT/Codex backend. Terminal siblings remain available in that view while coordination is active, but an all-terminal child history no longer leaves a permanent request tail. Codex agents obtain active state through `subagent_control list` and still receive terminal notifications. GPT-5.6 `auto` requests with this or any other volatile suffix compile with explicit placement so the suffix remains unmarked and stable anchors remain reusable.
...
For GPT-5.6 and later, direct public OpenAI Responses lowering supports `prompt_cache_options` and explicit `prompt_cache_breakpoint` fields on system, user, and local tool-result input blocks; assistant replay always remains unmarked `output_text`. Auto mode combines explicit stable-prefix markers with OpenAI's implicit latest-message marker; explicit mode disables the managed marker. Pre-5.6 public models use the compatible retention field. ChatGPT Codex requests are key-only for every model and emit no `prompt_cache_breakpoint`, `prompt_cache_options`, or `prompt_cache_retention`. Their cache, thread, client-request, and process-local WebSocket affinity all use the stable prompt-cache key. Request-shape tests do not establish live Codex reuse, which remains provider-controlled without a hit-rate guarantee. Direct OpenAI is Responses-only; OpenAI-compatible Chat remains available only for configured third-party deployments. Model family and route capability are gated independently because sending the wrong cache field can be rejected by the provider.
