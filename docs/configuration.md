...
Native computer use has no JSON/JSONC configuration or helper-path environment override. A packaged macOS executable resolves both the bare `ycoding-computer-helper` and `ycoding-computer-helper.app` beside itself. Release installation places the verified executable pair and app bundle in `~/.local/bin`. iTerm and Finder use the bare helper; desktop window inspection, capture, and input launch the app bundle. Grant that exact app Accessibility and Screen Recording in macOS Privacy & Security before desktop use. Its ad-hoc signature may require a fresh grant after rebuilding or updating.

For source development on macOS, run `bun run build:computer-helper` explicitly before `bun dev`. The build writes the ad-hoc-signed bare helper and app bundle to the repository-ignored `packages/core/.cache/computer-helper/` directory; Bun source execution resolves those fixed paths. Rebuilding or re-signing the app may invalidate its privacy grants. Runtime startup never compiles or installs either helper, and it never writes beside the user's Bun executable. Windows and Linux expose no native computer capabilities. See [Native computer use](./computer-use.md).
...
Daybreak is OpenAI's Trusted Access for Cyber program with two access levels, `daybreak_blue` and `daybreak_red`, selected per Session as `session.daybreak`; an absent value is off and requests keep standard safeguards. ChatGPT connections discover Daybreak availability from the authenticated Codex model catalog at startup and when the active connection changes. Each ordinary model entry advertising `daybreak_blue` or `daybreak_red` in `available_access_programs.cyber` carries an advertised `daybreak` list containing exactly those programs, so the model list stays one entry per model, the model picker shows ordinary models only, and Daybreak state is controlled by the toggle instead of by picking a special model. Discovery runs in the background with a three-second timeout; missing, unknown, invalid, or unavailable metadata never blocks ordinary model use. Changing the connection clears the previous account's discovery, and late responses cannot restore it. One toggle command, visible in the Session command palette and also available as the `/daybreak` slash command with the optional arguments `blue`, `red`, and `off`, controls the selection: without an argument it cycles off → blue → red → off, offering only programs the active model advertises and skipping any program it does not advertise. Turning a program on stores the selection for the Session durably as the `session.daybreak.set` event. When `session.daybreak` is set, the credential is ChatGPT OAuth on Codex backend routes, and the active model advertises that program in its `daybreak` list, requests carry `access_programs: { cyber: <program> }`; a program the active model does not advertise omits the field, failing closed to standard safeguards while the stored Session selection persists, and API-key credentials and custom providers omit the field (Codex parity). Discovery is not entitlement: no API-key entitlement, plan-name inference, or ChatGPT UI toggle is used, and the provider still enforces inference authorization.
...
The **Connect Chrome** command in both the Mini and full Session palettes creates a one-time code only when the user presses **p** and displays the connected loopback service address. Enter the code in the unpacked extension's popup. The popup uses `http://127.0.0.1:4096` by default; if the view shows another address, enter it once under **Local service address**. The extension remembers the address and reconnect credential, so pairing does not require the initiating Session to remain active. Pairing authorizes eligible existing and future tabs in that Chrome profile, including the active tab. The source-only manifest requests `tabs`, `tabGroups`, `alarms`, `debugger`, and `storage`, with host access limited to loopback. Omitted browser mode uses the paired profile; `mode: "owned"` opens Session-owned background tabs. Model-driven profile reads require site permissions without a standard access review; mutations require site permissions and fresh hard human review, and group changes also require control permission. Owned-tab open requires a human `once` hard review even under YOLO or goal automation. Approved site actions may cause incidental downloads; explicit download/upload access is not configured. See [Chrome browser bridge](./browser-extension.md) for ownership, badges, and safety limits.

The popup shows **Local service address**, **Pairing code**, and **Connect YCoding** only before pairing. It then shows a connection indicator, with **Forget pairing** to revoke browser access. A temporary outage retains the pairing and retries without another code; an explicit stop leaves it paused until **Retry connection** is chosen. The popup supports light and dark themes.
