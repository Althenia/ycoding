...
| model `.settings`, `.headers`, `.body`                                                  | JSON record, string record, JSON record                                                                         | unset                           | Model request overlays. For Codex, `settings.transport` may be `"websocket"` for explicit WebSocket opt-in or `"http"`/`"http-sse"` for HTTP SSE; omitted Codex transport uses HTTP SSE. |
...
| `compaction.context_safety_margin_tokens`                                               | non-negative integer                                                                                            | `30000`                         | Tokens reserved from the model input budget before context-pressure and model-switch fit calculations.                                             |
...
| `compaction.advisory`                                                                   | `false` \| `{ consider_percent, strongly_advised_percent }`                                                     | `{ 70, 90 }`                    | Disables automatic soft-pressure admission or sets ordered integer thresholds from 1 through 99 of the model context limit. Mandatory pressure at the smaller safe input hard cap is independent of this switch. |
...
Direct GPT-5.6+ Responses on supported direct OpenAI routes is **explicit-only**: it always sends `prompt_cache_options: { mode: "explicit", ttl: "30m" }`, disables the managed implicit breakpoint, and emits at most 50 explicit input-content candidates — one combined system-text marker when system text exists and the newest eligible non-volatile user and local tool-result boundaries within the remaining budget. Assistant replay remains unmarked `output_text`; marked local text results use `input_text` inside `function_call_output.output`, images use provider-native `input_image`, and non-image files use `input_file` with Base64 `file_data` and a filename. Tool definitions, tool calls, provider-executed tool results, and volatile messages receive no generated marker. Older public OpenAI models remain implicit. Non-Codex cache keys digest only the final system and tool definitions. ChatGPT Codex requests use each durable Session's raw Session ID for both `prompt_cache_key` and `session-id`; parent and child Sessions do not share this key, and it remains stable through native compaction. Codex is key-only for every model: YCoding emits none of `prompt_cache_breakpoint`, `prompt_cache_options`, or `prompt_cache_retention`. The public-route 1,024-token reusable-prefix contract does not apply to Codex because it emits no explicit boundary; only provider-reported `cached_tokens` and `cache_write_tokens` establish subscriber reuse. OpenAI-compatible gateways and unsupported model families omit GPT-5.6-only fields. This follows the provider migration guide: an explicit `prompt_cache_breakpoint` at the end of the stable prefix plus a stable `prompt_cache_key` lets suffix content change without invalidating the cached prefix, and `mode: "explicit"` avoids `1.25×` cache-write charges for the changing suffix.
...
{
  "compaction": {
    "context_safety_margin_tokens": 30000,
    "timeout_seconds": 60,
    "advisory": {
      "consider_percent": 70,
...
Soft thresholds are runtime-owned and use the configured percentages of the model context limit. A Session starts background compaction once when it rises from normal pressure to `consider`, and once more if it rises to `advised`. Each soft level remains latched after successful, deduplicated, or failed admission until pressure returns to normal; a later admission at the same level requires another 5% of the safe input budget. The hard input cap is the model context limit minus `context_safety_margin_tokens` (default `30000`) and independently triggers mandatory compaction. No cooldown or polling configuration is required.
