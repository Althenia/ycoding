...
ChatGPT connections discover Daybreak availability from the authenticated Codex model catalog at startup and when the active connection changes. Models explicitly advertising `daybreak_blue` or `daybreak_red` in `available_access_programs.cyber` receive separate `· Daybreak Blue` or `· Daybreak Red` picker entries. Their catalog IDs append `-daybreak-blue` or `-daybreak-red`; requests retain the underlying model ID and send the corresponding `access_programs.cyber` selection. Ordinary model entries remain separate. Discovery runs in the background with a three-second timeout; missing, unknown, invalid, or unavailable metadata does not create Daybreak entries or block ordinary model use. Changing the connection clears the previous account's discovery, and late responses cannot restore it. No API-key entitlement, plan-name inference, or ChatGPT UI toggle is used. The provider still enforces inference authorization.
