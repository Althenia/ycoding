...
| ChatGPT Codex Responses backend        | Subscription backend with a stable `prompt_cache_key` and a process-local WebSocket per YCoding Session. Clean strict-extension follow-ups may send `previous_response_id` plus only the new suffix over that socket while retaining `store: false`; reconnects and request-identity mismatches send full history. Every Codex model is key-only: YCoding emits no `prompt_cache_breakpoint`, `prompt_cache_options`, or `prompt_cache_retention`. The `session-id` affinity header matches the cache key; `thread-id` and `x-client-request-id` share the per-YCoding-Session provider identity. | Backend-controlled. Request-shape tests do not verify live cache reuse.                                                                                                                       | Reported cached input and cache-write tokens are normalized separately; omitted fields remain unreported. |
...
The public OpenAI API-key route and the ChatGPT/Codex subscription backend are distinct capabilities. Public GPT-5.6+ Responses can receive the explicit breakpoint and request-wide cache-option fields above. Every Codex model is key-only: YCoding emits none of `prompt_cache_breakpoint`, `prompt_cache_options`, or `prompt_cache_retention`. Its `session-id` header equals `prompt_cache_key`, and `thread-id` equals `x-client-request-id` using the existing per-YCoding-Session provider identity. Codex remains `store: false`; its `previous_response_id` optimization is scoped to one live process-local Session socket and is discarded on reconnect, restart, identity mismatch, non-terminal closure, or non-completed response. Those boundaries trigger canonical full-history replay rather than durable stored-Responses continuation. Codex retention and cache reuse remain provider-controlled and are not established by request-shape tests; no hit rate is guaranteed.
