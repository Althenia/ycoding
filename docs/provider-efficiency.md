...
| Direct OpenAI Responses GPT-5.6+ | Stable `prompt_cache_key`, one combined system-text marker, and every non-volatile user/assistant text boundary retained as an explicit marker. A marked assistant message uses `input_text` EasyInput blocks and otherwise retains `output_text`. Tools and tool results can remain in the cached prefix but receive no generated marker. `auto` retains OpenAI's managed latest-message breakpoint; OpenAI selects its latest three explicit writes. `explicit` disables that managed breakpoint; OpenAI selects its latest four explicit writes. Earlier markers remain provider read candidates across tool-heavy turns. | `prompt_cache_options.ttl: "30m"`, the provider's only supported minimum lifetime, is sent in `auto` and `explicit` modes; it is not a hard expiry and OpenAI has a separate 24-hour maximum. | Cached input and billable cache-write tokens are normalized separately when reported. |
| ChatGPT Codex Responses backend | YCoding sends a stable `prompt_cache_key` and otherwise uses backend-managed placement. Public-API cache options and breakpoints are omitted because ChatGPT subscription access is not the public Platform API contract. | Backend-controlled; public documentation does not specify subscription cache-control fields or retention. | Reported cached input and cache-write tokens are normalized separately; omitted fields remain unreported. |
