...
Prepared-request regression tests cover append-only chronological observations, preservation of a prior eligible explicit boundary when a volatile suffix changes hybrid mode to explicit, early/recent anchors as history grows, and exclusion of public cache controls on Codex. These checks verify wire construction, not cache residency, provider reads/writes, latency, or cost savings. A real-provider comparison must measure reported cache usage and input cost separately from observation growth; no cache-policy change or higher hit rate is implied by the projection change.
