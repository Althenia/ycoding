...
This document describes the YCoding provider-efficiency contract: request amplification, prompt caching, OpenAI Responses state, diagnostics, privacy boundaries, and reproducible verification. Provider-side Responses compaction, local selective compaction, and prompt caching are separate mechanisms.
...
Responses completion snapshots are reconciled with streamed text before silence detection. An answer present in a finalized text part, message item, or terminal response output is retained without a redundant recovery request. Usage and reasoning remain provider-reported; text reconstruction does not infer token counts or change cache options.

HTTP response-body read failures and normalized streams ending without a terminal event retain transport classification. Before observable assistant output, the Session runner applies the bounded physical-attempt schedule to those failures and recognized transient provider-error events. Each HTTP/SSE Physical Attempt, including first attempts and physical retries from Session or helper calls, uses one isolated connection through Fetch `keepalive: false` and `Connection: close`. This changes connection setup only: it does not rotate the prompt-cache key, imply a cache miss, change provider cache controls, or establish backend reuse.

After observable output or tool evidence, a transport read failure or retryable provider-internal failure closes the Step durably across provider routes. The failed Physical Attempt is never replayed. After any eligible provider-requested delay, a pending steer wins the safe boundary; otherwise one separate full logical recovery request may reload durable history and completed tool results with stored Responses continuation disabled. This recovery is not a Physical Attempt retry or `fallback`, obeys the current agent Step allowance and tool rules, and increments both logical-request and physical-attempt diagnostics once. Without pending input, another post-output read or internal failure in recovery cannot start a third automatic Step. A successful recovery or newly promoted input returns execution to normal mode, allowing a later distinct failure to receive its own bounded recovery; a steer admitted during failing recovery is promoted after that recovery Step closes. Terminal-silence recovery remains the separate tools-disabled correctness path above. WebSocket heartbeat eviction and retry remain separate transport behavior, and Codex HTTP/SSE and WebSocket never fall back to each other.
...
| `helper_models.compaction.main`, `.subagent` | `session`  | Local compaction selects the helper model; an explicit configured value wins over an agent-pinned model, while `session` uses the owner Session's model precedence. Codex remote compaction uses the owner's model and invokes the helper only if remote compaction fails. |
...
| `prompt_cache.openai_extended_retention`     | `false`    | Does not request `24h` retention for pre-GPT-5.6 OpenAI families or Meta Muse Spark unless explicitly enabled.                                                                                         |
...
Set `title` to `model` when model-generated titles are required. Explicit `/goal <text>` calculation always uses the hidden goal agent; `agents.goal.system` supplies its pre-prompt. Model precedence is `agents.goal.model`, then `efficiency.helper_models.goal`, then the current Session model. An invalid configured model fails calculation rather than falling back. Ordinary prompts and goal resume make no new goal-calculation request. Selective-compaction manifest generation remains model-assisted and uses the selected compaction helper, but activation is governed by mechanical validation and immutable context revisions rather than summary replacement.
...
The namespace does not include the Session ID, context revision, or ordinary message history, so equivalent Sessions record the same namespace and system/tool digests. The provider-visible `prompt_cache_key` is Session-scoped on every key-carrying route (direct OpenAI, ChatGPT Codex, GitHub Copilot, OpenRouter, and OpenAI-compatible routes): it is derived from the Session ID and the namespace. Parallel Sessions, including sibling subagents with identical namespace inputs, therefore never pool their request rate under one provider key, which keeps each key within OpenAI's per-key routing guidance and matches the per-conversation key of the official Codex client. Cross-Session reuse of an identical prefix remains provider-controlled. OpenRouter additionally receives a separate `session_id` provider-session identity for sticky routing. The ChatGPT Codex backend sends the Session-scoped key as its cache, thread, and client-request identity; the explicit WebSocket route also uses it for process-local socket affinity. Prompt-cache keys do not rotate on a timer or in response to a low provider-reported hit ratio. Context-revision or history-tail changes alone do not change the owner namespace, key, system digest, or tool digest. Switching provider, model, variant, or fixed transport route selects that route's distinct namespace; switching back restores the prior key and system/tool digests when every namespace input is byte-identical. Restoring the complete provider-visible prefix additionally requires the selected history and any plugin-owned volatile suffix bytes to match. Each compaction job instead uses a deterministic taskless child Session, the hidden `compaction` agent, a separate provider identity and request ledger, and an internal `compaction` cache scope. Helper traffic therefore cannot share an ordinary Session-step cache key or contribute to the owner's provider-request ledger. Provider, model, variant, policy revision, permissions, final system bytes, or final tool bytes still legitimately change the corresponding key.
...
Hooked provider-visible tools are ordered by name and their JSON schema object keys are canonicalized before request compilation. Their tool digest and emitted schema therefore agree even when hook discovery or schema insertion order differs. Tool-output messages retain their chronological position.
...
For text-only models, the image-analysis fallback reuses each successful description per image content digest, vision model, and effective prompt for the life of the runtime process. Every later step therefore replays byte-identical description text in the user message and makes no further vision request for that image. A failed analysis is not reused, and a runtime restart analyzes each image once more.

## Provider capability matrix

| Provider route                         | Placement and identity                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Retention behavior                                                                                                                                                                            | Usage diagnostics                                                                                         |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Anthropic Messages | Inline cache controls on tools, system, and two rolling message anchors: the newest cacheable message and the last cacheable message before the latest assistant message, which is the previous request's final anchor. A large parallel tool fan-out therefore cannot move both anchors beyond the provider's 20-block lookback. Public API-key requests use this direct route without Claude Code request translation. Signed thinking, including empty thinking that carries only a signature, and redacted thinking are replayed unchanged. | Adaptive five-minute or one-hour TTL when the model has a published extended-TTL profile. | Provider cache reads and writes are preserved separately. Writes reported in the one-hour `cache_creation` bucket are priced at twice the base input rate; aggregate-only write reports use the catalog write price. |
...
| OpenRouter                             | Defaults to the stateless `/responses` route with a Session-scoped `prompt_cache_key` and a separate `session_id` provider-session identity for sticky routing. Replayed reasoning items, including encrypted content, assistant `phase`, and tool metadata use the OpenAI Responses metadata namespace, so follow-ups keep them. Top-level automatic `cache_control` is sent only for Anthropic models. OpenAI GPT-5.6-and-later models receive `input_text` breakpoints, `include: ["reasoning.encrypted_content"]`, and `reasoning.context: "all_turns"`; request-wide `prompt_cache_options` is sent only in explicit mode because automatic caching is OpenRouter's default. Explicit AI SDK providers retain their bounded inline marker behavior. Non-Anthropic models receive no Anthropic cache policy.                                                                                                                                                                                                                                           | Adaptive five-minute or one-hour Anthropic TTL on profiled Anthropic models and five minutes on other Anthropic models; OpenAI GPT-5.6-and-later prefixes keep OpenRouter's documented 30-minute minimum; otherwise provider-controlled. | Reported cache categories and request cost are normalized.                                                |
| Direct OpenAI Responses before GPT-5.6 | Implicit prefix caching with a stable Session-scoped prompt-cache key.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Optional `24h` retention only for allowlisted model families.                                                                                                                                 | Cached input and cache-write tokens are normalized when reported.                                         |
| Direct OpenAI Responses GPT-5.6+       | Public API-key route with a stable Session-scoped `prompt_cache_key`, `reasoning.context: "all_turns"`, one combined system-text marker, and the most recent eligible non-volatile user and local tool-result text boundaries. Breakpoints are emitted only on Responses `input_text` blocks; assistant replay always uses `output_text` and is never marked. A marked local tool result uses `function_call_output.output` content blocks without changing structured media. `auto` reserves one of OpenAI's latest 50 read candidates for the managed implicit breakpoint unless a volatile suffix makes that request explicit. `explicit` disables the managed breakpoint and uses only explicit candidates. | `prompt_cache_options.ttl: "30m"`, the provider's only supported minimum lifetime, is sent in `auto` and `explicit` modes; it is not a hard expiry and OpenAI has a separate 24-hour maximum. | Cached input and billable cache-write tokens are normalized separately when reported.                     |
| ChatGPT Codex Responses backend        | Subscription backend using HTTP/SSE by default (`openai-codex-responses`) and a distinct explicitly selected WebSocket route (`openai-codex-websocket-responses`). Each configured HTTP/SSE Physical Attempt uses an isolated connection; WebSocket affinity remains separate and neither route falls back to the other. Clean WebSocket strict-extension follow-ups may send `previous_response_id` plus only the new suffix while retaining `store: false`; a failed exchange keeps the last successful anchor so an ordinary retry repeats that incremental request. Every Codex model is key-only: YCoding emits no `prompt_cache_breakpoint`, `prompt_cache_options`, or `prompt_cache_retention`. The `session-id`, `thread-id`, and `x-client-request-id` headers all match the Session-scoped cache key. Reasoning models request `include: ["reasoning.encrypted_content"]` with or without a selected effort variant, so stateless replay keeps their reasoning items. | Backend-controlled. Connection isolation does not establish or change cache reuse; request-shape tests do not verify live reuse.                                                               | Reported cached input and cache-write tokens are normalized separately; omitted fields remain unreported. |
| Meta Muse Spark | Native Responses route (`openai-responses`, provider `meta`) with `store: false` stateless replay, a stable Session-scoped `prompt_cache_key`, and `include: ["reasoning.encrypted_content"]` so encrypted reasoning items are replayed on every follow-up. Direct-OpenAI stored continuation, GPT-5.6 breakpoints, request-wide cache options, and server compaction are not sent. | Automatic prefix caching. The `24h` `prompt_cache_retention` hint is sent only when `openai_extended_retention` is enabled. | Cached input from `input_tokens_details.cached_tokens` is normalized as cache reads. |
| GitHub Copilot Responses | Stateless full-history replay with `store: false` and a stable Session-scoped `prompt_cache_key`. Models that advertise reasoning effort request `include: ["reasoning.encrypted_content"]` with or without a selected variant. Follow-ups replay retained encrypted reasoning and local tool outputs directly without provider-stored item references. Models outside the `gpt-5`, `gpt-5.1`, and `gpt-5.2` families send `context_management` server compaction at 90% of the advertised prompt limit, or 50,000 tokens when the limit is unreported. The returned encrypted compaction item is stored privately, replayed on later stateless requests, and everything before it is omitted. | Provider prefix caching may apply; stored Responses continuation is disabled. Server compaction replaces the covered history with the encrypted item. | Cached input and cache-write tokens are normalized separately when reported; omitted fields remain unreported. Replay is not treated as continuation. |
| GitHub Copilot Chat (Claude) | Claude models that Copilot serves only through Chat Completions receive message-level `copilot_cache_control: { type: "ephemeral" }` on the system prompt and the two rolling message anchors, at most four markers per request. GPT, Gemini, and Grok Chat models receive no markers. | Provider-controlled; the Chat field carries no TTL. | Provider-reported cached input is normalized. |
| OpenAI-compatible gateways | Stable provider identity where supported; unsupported direct-OpenAI fields are omitted. | No direct OpenAI retention field is assumed. | Provider-reported usage is normalized without inventing unsupported capabilities. DeepSeek Chat cache hits reported only as top-level `prompt_cache_hit_tokens` are normalized as cache reads. |
...
Runpod Ollama Jobs report cached input reads when the worker returns `prompt_eval_cached_count` alongside `prompt_eval_count`. YCoding clamps inconsistent cached counts to the inclusive evaluated input total and retains missing counts as unreported. Provider-reported `prompt_eval_duration`, `eval_duration`, and `load_duration` are recorded as optional nanosecond timings in the request ledger and latest Session usage summary; they are never fed back into a model prompt. Runpod vLLM uses `usage.prompt_tokens_details.cached_tokens` when present. Neither route sends OpenAI cache controls; cache retention and worker placement belong to the serving deployment. The current Runpod route exposes no verified inference-worker identity or affinity control.
...
Automatic TeamView observations exclude operational revisions and timestamps while retaining decision-relevant child state and questions. Stable selection and canonical JSON prevent metadata-only churn from growing model history; explicit inspection and durable task records retain that metadata. Only a changed projected observation is appended, and existing messages are never regenerated from the current task table. See [durable runtime observations](runtime.md#durable-runtime-observations). Session-state goal/todo/permission updates remain intact.

Prepared requests preserve append-only chronological observations, a prior eligible explicit boundary when a volatile suffix changes hybrid mode to explicit, early and recent anchors as history grows, and exclusion of public cache controls on Codex. A real-provider comparison must measure reported cache usage and input cost separately from observation growth; YCoding guarantees wire construction, not cache residency, provider reads or writes, latency, or cost savings.
