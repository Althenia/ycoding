...
| GitHub Copilot Responses | Stateless full-history replay with `store: false` and a stable Session-scoped `prompt_cache_key`. Models that advertise reasoning effort request `include: ["reasoning.encrypted_content"]` with or without a selected variant. Follow-ups replay retained encrypted reasoning and local tool outputs directly without provider-stored item references. | Provider prefix caching may apply, but stored Responses continuation is disabled. | Cached input and cache-write tokens are normalized separately when reported; omitted fields remain unreported. Replay is not treated as continuation. |
...
The public OpenAI API-key route and the ChatGPT/Codex subscription backend are distinct capabilities. Public GPT-5.6+ Responses can receive the explicit breakpoint and request-wide cache-option fields above. Every Codex model is key-only: YCoding emits none of `prompt_cache_breakpoint`, `prompt_cache_options`, or `prompt_cache_retention`. Its `session-id`, `thread-id`, and `x-client-request-id` headers all equal the Session-scoped `prompt_cache_key`, matching the official Codex client, which sends a per-conversation `session-id` because ChatGPT derives cache affinity from that header. Codex uses HTTP/SSE by default; only explicit `transport: "websocket"` enables process-local WebSocket affinity. Codex remains `store: false`; its WebSocket `previous_response_id` optimization is process-local. A failed WebSocket exchange retains the last successful anchor for an exact incremental retry, while restart or request-identity mismatch sends canonical full history because no compatible anchor exists. Codex retention and cache reuse remain provider-controlled and are not established by request-shape tests; no hit rate is guaranteed.
