...
The responsive layout reflows at desktop, tablet, and mobile breakpoints. At compact widths, the Activity header keeps Sessions, Conversation, Activity, and Settings navigation visible with touch-sized links. Public and remote routes expose a main landmark and mark the current navigation destination; dialogs retain keyboard focus inside their controls, and Settings radio groups support arrow, Home, and End selection with one tab stop per group. Light and dark themes, keyboard-accessible approvals, bounded tool/terminal output, and an accessible composer are required on each surface. Unsent prompt drafts belong to their selected Session and remain available through reconnects. Offline mode never queues remote mutations.
